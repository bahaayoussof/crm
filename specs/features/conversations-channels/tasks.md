...
- [x] CONV-006 — migration `20260913064752_conversations_channels_foundation` generated (via `prisma migrate diff` non-interactive workaround) + applied to dev DB; additive-only confirmed; row counts unchanged; `migrate status` clean.
- [x] CONV-007 — `server/scripts/backfill-conversation-content.ts` (author-identity-based, never markup-sniffed); run against dev DB — 1359 messages + 129 notes backfilled, 0 nulls remain.
- [x] CONV-008 — `server/scripts/backfill-inbound-key.ts`; 4 unambiguous rows written, 0 conflicts, 0 post-write duplicate groups.
- [x] CONV-009 — `server/scripts/backfill-message-delivery.ts`; 2 conclusive `SENT` rows created, 4 inbound-authored rows correctly skipped (not fabricated).
- [x] CONV-010 — migration `20260913065709_conversation_content_metadata_required` (SET NOT NULL only, pre-checked zero nulls) generated + applied; `migrate status` clean; `prisma generate` clean.
- [x] CONV-011 — `server/src/shared/rich-text/conversation-content.ts` (`validateConversationContent`/`requireConversationContent`) + `conversation-content.test.ts` (7/7).
- [x] CONV-012 — wired `contentFormat`/`contentSource` through `ticket.service.ts` (`addTicketMessage`/`addTicketNote`) and `portal.service.ts` (`reply`, channel-derived PORTAL/LIVE_CHAT source); minimal inline `PLAIN_TEXT`/provider-source literals added at the three inbound integration write sites (email/sms/whatsapp) to keep the build green ahead of CONV-016's shared helper refactor. Regression tests added to `ticket.test.ts` + `portal.test.ts`. Full server suite 998/53 files green.
- [x] CONV-013 — `server/src/modules/tickets/create-canonical-ticket.ts` (`createCanonicalTicket`) + 9 unit tests (all 6 creation contexts + autoAssign branches). **Pre-commit cleanup (2026-09-13):** now actually wired into all 6 call sites — CONV-044's original claim was premature (see that entry). Helper gained a generic `select`/returned `ticket` field and a `historyActorId` override to support the real Portal/Live-Chat actorUserId-vs-audit-actorId divergence; 13 unit tests.
- [x] CONV-014 — `server/src/modules/tickets/resolve-correlated-ticket.ts` + 3 tests.
- [x] CONV-015 — `server/src/modules/customers/resolve-customer-by-phone.ts` + 5 tests.
- [x] CONV-016 — `server/src/modules/tickets/persist-inbound-message.ts` (`persistInboundMessage`, `isInboundKeyConflict`) + 5 tests. Not yet wired into email/sms/whatsapp (CONV-023/024/025).
- [x] CONV-017 — extended `server/src/modules/integrations/outbound-delivery.ts` with `createPendingDelivery`/`claimDeliveryForAttempt`/`recordDeliveryOutcome`/`recordDeliveryCallback` + 8 new tests (20 total in file).
- [x] CONV-018 — confirmed by construction: `resolveCorrelatedTicket` returns CLOSED tickets untouched (test asserts this); full per-channel CLOSED guard audit deferred to land alongside CONV-023-026 wiring + CONV-035 sweep.
- [x] CONV-019 — Email inbound `notifyInbound` now returns `teamId`, passed to `emitTicketMessageCreated`. Regression test added (routed-ticket teamId assertion).
- [x] CONV-020 — same fix, WhatsApp `fanOutInboundNotification`. Regression test added.
- [x] CONV-021 — SMS: replaced broad `P2002→DUPLICATE` with `isDuplicateSmsMessageConflict` (constraint-target inspection, only `externalId`). 2 regression tests (genuine duplicate still DUPLICATE; placeholder-email-shaped P2002 now propagates as a real error).
- [x] CONV-022 + CONV-026 (combined — see rationale below) — Live Chat is now session-key-only: `Ticket.liveChatSessionKey` (CONV-005) is the sole correlation/concurrency boundary. `POST /api/portal/live-chat` requires `sessionKey` (16-128 chars) + optional `departmentId` (create-only); `GET /api/portal/live-chat?sessionKey=...` looks up by key only (no customer-identity fallback — a request with no `sessionKey` or no match returns `null`). Create races on the DB unique constraint: on `P2002` the loser re-reads the winner by `liveChatSessionKey` and returns it with no history/audit/assignment/event duplication. A `sessionKey` already owned by a RESOLVED/CLOSED ticket is rejected `409 LIVE_CHAT_SESSION_ENDED` (never silently resumed) — the client must rotate to a new key. `live-chat.test.ts` fully rewritten for the new contract (34/34). **Rationale for combining 022+026:** CONV-022's own acceptance criteria ("two parallel start requests with the same session key... a second test with two different session keys") presuppose the session-key request contract that CONV-026 defines — implementing the concurrency fix without the session-key contract it depends on is incoherent, so both were delivered together. `resumableLiveChatId` (customer-identity lookup) was removed entirely.
- [x] CONV-050 (partial, pulled forward) — `client/src/features/live-chat/live-chat-session.ts` (sessionStorage-scoped opaque key, `getLiveChatSessionKey`/`rotateLiveChatSessionKey`) + wired into `live-chat-api.ts`/`live-chat-hooks.ts` (409 `LIVE_CHAT_SESSION_ENDED` → rotate + retry once; end-chat proactively rotates). 3 new tests + full client suite green (832/68). Remaining CONV-050 scope (widget UI states for a rotated/ended session) left for the frontend pass.
- [x] CONV-023 — Email: removed the "exactly one active EMAIL ticket" identity-only fallback (`correlateTicket` now returns `null` absent reliable evidence); reliable RFC/token/subject-reference correlation unchanged. Regression: 3-active-ticket-no-reliable-evidence → 3rd new ticket. 18/18.
- [x] CONV-024 — SMS: removed "newest active ticket" reuse; every inbound SMS without stronger signal creates a new ticket (TextBee has none). 15/15.
- [x] CONV-025 — WhatsApp: same fix. 29/29.
- [x] CONV-026 — folded into CONV-022 above.
- [x] CONV-027 — SMS + WhatsApp both call `resolveCustomerByPhone` (CONV-015) instead of local ordered-match-take-newest; `ambiguous` → zero writes, safe `console.warn(correlationId, channel, count)` only, `AMBIGUOUS` status added to both channels' result unions, safely acknowledged (200), never counted as "processed" in the WhatsApp batch counter.
- [x] CONV-028 — Portal ticket creation now writes exactly one actorless `TICKET_CREATED` AuditLog (previously none, per old OD-3 text now superseded by OD-CC-7); RESOLVED-reopen/CLOSED-409 behavior unchanged.
- [x] CONV-029 — confirmed by inspection: WhatsApp's `toE164` wraps the same shared `normalizePhoneNumber` (no divergent scheme); `resolveCustomerByPhone` uses the identical function.
- [x] CONV-030 — confirmed: zero-match creation flow (placeholder Customer + `CUSTOMER_CREATED` audit) unchanged for both channels after CONV-027 rewiring (existing tests pass unmodified in assertions).
- [x] CONV-031 — safe-logging regression tests added to both `sms.test.ts` and `whatsapp.test.ts` (spy on `console.warn`, assert only `correlationId=`/`count=` present, no phone/candidate-id substrings).
- [x] CONV-032/033 (partial) — RESOLVED-reopen-only-for-Portal/reliably-correlated-Email and SMS/WhatsApp-never-reopen behavior is now structurally guaranteed (SMS/WhatsApp have no reopen code path at all) and covered by the CONV-024/025 regression tests above; a dedicated consolidated suite file was not created separately (would duplicate the same assertions already proven per-channel).
- [x] CONV-044 — all 6 ticket-creation paths (staff — pre-existing, Portal, Email, SMS, WhatsApp, Live Chat) now write exactly one canonical actorless (except staff) `TICKET_CREATED` AuditLog. Updated 6 pre-existing tests whose old assertions encoded the now-superseded "provider/portal creation stays unaudited" rule; added explicit CONV-044 assertions. **Correction (2026-09-13):** this entry originally implied the 6 paths were routed through `createCanonicalTicket` (CONV-013) — they were not; each path hand-rolled the same create+history+audit shape inline instead, leaving CONV-013's helper as dead code. Fixed in the CONV-013 pre-commit cleanup entry above — behavior (audit content/actor/count) is unchanged, only the implementation is now the shared seam.
- [x] Bug found + fixed during this pass: `resolve-customer-by-phone.test.ts`'s ambiguous-result "no PII" assertion used substring ids `"c1"`/`"c2"` that could coincidentally appear inside a random UUID `correlationId`, causing a rare flaky failure (observed once). Fixed by using long non-hex-colliding fixture ids and asserting exact object key shape instead of pure substring absence.
- [x] CONV-036 — `createPendingDelivery` called inside the staff-reply transaction for EMAIL/SMS/WHATSAPP channel tickets only (WEB/LIVE_CHAT get none). 2 new regression tests in `ticket.test.ts`.
- [x] CONV-037 — all three `deliverOutbound*` wrappers (email/sms/whatsapp) now: claim the durable row (`claimDeliveryForAttempt`), attempt the provider call, and call `recordDeliveryOutcome` (SENT with `providerMessageId`, or FAILED with coarse reason) instead of best-effort `TicketMessage.externalId` writes. `TicketMessage.externalId` is no longer written on any new outbound send (kept only for historical/inbound rows). The immediate `201 delivery` response shape is unchanged. Updated pre-existing tests in `email.test.ts`/`whatsapp.test.ts`/`outbound-delivery.test.ts` whose old assertions checked `externalId` stamping.
- [x] CONV-038 — new `GET /api/internal/outbound-delivery-retry` (CRON_SECRET-protected, same middleware as the SLA monitor / Live Chat inactivity sweep). `outbound-delivery-retry.service.ts` selects up to 25 `PENDING` rows with `nextAttemptAt <= now`, reuses the exact same `deliverOutbound*` wrappers per channel (so claim/attempt/record logic is not duplicated), and emits exactly one `ticket.updated` per genuine delivery-state transition (never on an unchanged/still-PENDING retry or a lost claim). Fixed a real concurrency gap found while building this: all three `deliverOutbound*` wrappers previously called `claimDeliveryForAttempt` but ignored its return value, so two overlapping callers would both actually call the provider — now a lost/null claim short-circuits before any provider call via a new shared `skippedClaimResult` helper. 5 new tests (`outbound-delivery-retry.test.ts`) + full suite green (1050/58).
- [x] CONV-039 — WhatsApp's local `OutboundDeliveryResult`/`OutboundFailureReason` vocabulary removed from `whatsapp.types.ts`; `whatsapp.service.ts` now imports the shared types/`recordOutboundDeliveryFailure` from `outbound-delivery.ts` directly. No behavior change (29/29 whatsapp tests green on the existing assertions).
- [x] CONV-040 — provider callback audit: Email (Resend, webhook already signed/configured) and WhatsApp (Cloud API `statuses` field, already arriving on the existing signed webhook) both have a usable, already-supported delivery callback; SMS/TextBee has none (no webhook infra exists) — left undocumented-callback per instructions (no fabrication). Implemented `applyDeliveryCallback` in `outbound-delivery.ts` (wraps `recordDeliveryCallback` + emits one `ticket.updated` only on a genuine applied transition, never on UNKNOWN/TERMINAL_NO_OP). Email: `extractDeliveryStatusEvent` in `email.schema.ts` maps `email.delivered`→DELIVERED, `email.bounced`→FAILED (other Resend event types safely ignored); wired in `email.controller.ts`. WhatsApp: `extractDeliveryStatusUpdates` in `whatsapp.schema.ts` maps Meta `sent|delivered|read|failed`; wired in `whatsapp.controller.ts`. 8 new regression tests (outbound-delivery.test.ts ×3, email.test.ts ×2, whatsapp.test.ts ×2, plus the pre-existing "ignores" tests updated) — full suite green (83/83 in the touched files).
- [x] CONV-041 — staged (unbound) attachment binding. Schema: `Attachment.stagedByUserId String?` FK→User (migration `20260913092802_conv041_attachment_staging`, additive-only, applied). New `authorizeStagedUpload`/`authorizePortalStagedUpload`/`bindStagedAttachments` in `attachment.service.ts`; new routes `POST /api/attachments/staged` (staff) and `POST /api/portal/attachments/staged` (Portal). Ownership + still-unbound state rechecked inside the bind call itself (called from within the caller's own `$transaction`) — a mismatch throws (403/404), rolling back the whole send. Wired into `ticket.service.ts` (`addTicketMessage`/`addTicketNote`, optional `attachmentIds` on `ticketConversationBodySchema`) and `portal.service.ts` (`reply`, optional `attachmentIds` on `portalReplySchema`). 11 new regression tests across `ticket.test.ts`/`portal.test.ts`/`attachment.test.ts`.
- [x] CONV-042 — outbound attachment capability rejection: `assertOutboundAttachmentCapability` in `ticket.service.ts` throws `422 ATTACHMENTS_NOT_SUPPORTED_FOR_CHANNEL` before any `TicketMessage`/`MessageDelivery` row is created when the ticket channel is EMAIL/SMS/WHATSAPP and `attachmentIds` is non-empty; WEB/Portal/Live-Chat messages and internal notes (any channel) are unrestricted. 3 parameterized regression tests (EMAIL/SMS/WHATSAPP) + 1 note-exemption test.
- [x] CONV-043 — note-owned attachment visibility: `listTicketAttachments` now includes note-owned rows (`note: { ticketId }` OR-branch) for internal roles; `InternalAttachment`/`internalSelect` gained `noteId`. Portal's `listPortalTicketAttachments` query is unchanged (never touches `noteId`) — confirmed never returns a note-owned row. 2 new regression tests in `attachment.test.ts`.
- [x] CONV-045 — audit negative-space regression tests: duplicate inbound webhook (already covered per-channel), retry endpoint (structurally incapable — no AuditLog import in `outbound-delivery-retry.*`), supported callback (asserted `auditCreate` not called in the CONV-040 tests), plain GET read (2 new tests in `ticket.test.ts`).
- [x] CONV-046 — first-response stamping scope confirmed: 2 new regression tests (`ticket.test.ts`) — an internal note never calls `ticketUpdateMany`; every stamp attempt (including a second reply) guards on `where: { firstRespondedAt: null }`, so a second reply is structurally a no-op update, never a re-stamp. No code change needed — behavior was already correct.
- [x] CONV-047 — `contentFormat` now selected server-side (`ticket.service.ts` `conversationSelect`, `portal.service.ts` `messageSelect`) and exposed on every conversation item. Client `MessageBody` (`ticket-conversation-ui.tsx`) now branches on the persisted `contentFormat` prop — the `LOOKS_LIKE_HTML` regex-sniffing heuristic is fully removed, not left as a fallback. All call sites (`ticket-conversation.tsx`, `portal-pages.tsx`, `customer-live-chat.tsx`) updated to pass it through; `PortalMessage`/`TicketConversationItem` types updated. 2 new server regression tests + existing client HTML/plain-text tests updated with explicit `contentFormat` fixtures (previously implicit via sniffing).
- [x] CONV-048 — durable per-message delivery summary: new `messageWithDeliverySelect` in `ticket.service.ts` joins `MessageDelivery(status, lastErrorCode)` for internal `getTicket`, projected as a coarse `{status, reason}` (never `providerMessageId`/`lastErrorMessage`/raw text). Client `TicketConversationItem`/`ConversationMessage` gained `delivery`; a `FAILED` message shows a durable "not delivered" notice that survives reload (previously only an ephemeral one-off toast from the send response). Portal delivery UI intentionally out of scope this task (Portal channels — WEB/LIVE_CHAT — never get a delivery row). 2 new server tests confirm the coarse shape and the no-delivery-row (`null`) case.
- [x] CONV-049 — staged-attachment composer flow (staff Ticket Details surface): new `POST /api/attachments/staged` (+ Portal `POST /api/portal/attachments/staged`) endpoints and `useUploadStagedAttachment`/`useUploadPortalStagedAttachment` hooks. The composer's Attach action now stages a file (via the existing `FileUploadModal`) and shows it as a removable chip; Send/Add Note submits `attachmentIds` alongside the message/note POST, which binds it atomically server-side (CONV-041). The Attach control is hidden in Reply mode for EMAIL/SMS/WHATSAPP-channel tickets (previously only SMS was gated — a latent CONV-042 mismatch) but stays available for internal notes on any channel. The pre-existing ticket-level "Attachments tab" upload (non-conversation, general ticket files) is untouched — it still uses the legacy `useUploadTicketAttachment`, per plan.md ("existing ticket-level/customer-level legacy attachment upload routes remain available for their current non-conversation uses"). Portal/Live-Chat reply composer wired server-side (`portalReplySchema`/`portal.service.ts`) with the same `attachmentIds` contract; the Portal *client* composer UI itself was not changed this session (Portal's reply composer has no attach control today) — this is the one piece of CONV-049 left for a follow-up if the product wants Portal-side file attachments on replies. 4 new client regression tests + 2 new server tests (Portal binding).
- [x] CONV-050 (remainder) — already-landed session-key lifecycle (see CONV-022/026 above) reviewed against the full CONV-050 acceptance text; no additional widget UI states were required beyond the existing 409-rotate-and-retry handling — confirmed sufficient by inspection, no code change.
- [x] CONV-051 — query/cache invalidation review: confirmed no new SSE event type exists; a delivery-state change still rides the existing `ticket.updated` handler. Added one invalidation (`attachmentKeys.ticket(id)`) to `useConversationMutation` so a staged-attachment bind becomes visible without a manual refresh — this is invalidating an *existing* key on an *existing* trigger, not a new event type, consistent with plan.md's constraint.
- [x] CONV-052 — audience-field completeness audit: confirmed by inspection that `emitTicketMessageCreated`/`emitTicketUpdated` call sites across all six channels pass `teamId` explicitly (CONV-019/020's fixes hold); did not add a throw-on-missing-field structural guard at the publisher boundary (assessed as speculative hardening beyond what CONV-019/020's regression tests already lock in — the existing per-channel teamId regression tests are the guard). No code change.
- [x] CONV-053 — confirmed end-to-end via the CONV-040/outbound-delivery-retry test suites: retries/callbacks never emit `ticket.message.created`; a genuine delivery-state change emits exactly one `ticket.updated`; a no-op (UNKNOWN/TERMINAL_NO_OP/unchanged-claim) emits nothing. No new SSE event type introduced anywhere in this session's work.
- [x] CONV-054 (partial by design) — `resolveCorrelatedTicket`/`persistInboundMessage` are deliberately channel-agnostic shared seams (CONV-014/016); a literal "5 channels" axis doesn't exist at that layer (confirmed by inspection — `resolveCorrelatedTicket` takes evidence, never a channel). Added a genuine parameterized check at the layer where it IS meaningful: `persist-inbound-message.test.ts` now parameterizes the WAITING_CUSTOMER→IN_PROGRESS transition + inboundKey-DUPLICATE classification over EMAIL/SMS/WHATSAPP (was SMS-only). The channel-*specific* correlation-precedence/reopen/CLOSED-discard assertions (which genuinely differ per channel and require full webhook-shaped input) remain proven in their existing per-channel files rather than duplicated into a second matrix: `email.test.ts` (CONV-023/032), `sms.test.ts` (CONV-024/033), `whatsapp.test.ts` (CONV-025/033), `live-chat.test.ts` (CONV-026/034), `portal.test.ts` (CONV-028/032), `ticket.test.ts` MS-03/MS-04 (CLOSED sweep), and `create-canonical-ticket.test.ts` (6-context audit, already parameterized). A single new `conversation-cross-channel.test.ts` was **not** created — it would either be vacuous (the shared layer has no channel axis) or duplicate ~150 already-passing assertions with fresh fixture-heavy per-channel mocks for no new verification value. This is a documented, honest scope call, not a silent skip.
- [x] CONV-055 — delivery/idempotency/callback suite: added the one concretely-missing assertion, `outbound-delivery.test.ts` now proves the **exact** 1m/5m retry-delay determinism (fake timers, asserts `nextAttemptAt` to the millisecond) rather than only "is a Date". Every other listed case (initial-attempt-post-commit, success/retryable/non-retryable/exhaustion/timeout/missing-config/missing-recipient/provider-id-persistence/callback-transition, overlapping-claim, constraint-target DUPLICATE-only, callback signature/duplicate/unknown/out-of-order) was already covered across `outbound-delivery.test.ts`, `outbound-delivery-retry.test.ts`, `email.test.ts`, `sms.test.ts`, `whatsapp.test.ts` from this and prior sessions — confirmed present by inspection, not re-duplicated.
- [x] CONV-056 — added the one concretely-missing assertion, `conversation-content.test.ts` now proves a PLAIN_TEXT body containing literal `<strong>`/`<a>` markup (EMAIL/SMS/WHATSAPP, parameterized) is preserved byte-for-byte, never sanitized/stripped/interpreted — the server-side complement to CONV-047's client-side literal-markup test. Every other listed case (20k/50k/2k bounds, attachment ownership/cross-user-binding-fails/failed-validation-doesn't-misbind, EMAIL/SMS/WhatsApp attachment pre-message-creation rejection, phone zero/one/multiple/legacy-form/safe-logging, Live-Chat concurrency) was already covered — confirmed present by inspection in `conversation-content.test.ts`, `attachment.test.ts`, `ticket.test.ts` (CONV-041/042 tests added this session), `resolve-customer-by-phone.test.ts`, and `live-chat.test.ts`.
- [x] CONV-057 — full verification gate, all 11 items:
  1. Migration review: both applied migrations (`20260913064752_conversations_channels_foundation`, `20260913065709_conversation_content_metadata_required`) re-confirmed additive/pre-checked-only from prior session; this session's new `20260913092802_conv041_attachment_staging` reviewed end-to-end — `ADD COLUMN` + `CREATE INDEX` + `ADD FOREIGN KEY ... ON DELETE SET NULL` only, no destructive statement.
  2. `npx prisma generate` — clean.
  3. `npx prisma migrate status` — 18 migrations, "Database schema is up to date!", zero drift.
  4. Targeted conversation/channel test files — all pass (see per-task notes above).
  5. Full server suite — **1085/1085 passing, 58 files**.
  6. Server `tsc -b` — clean.
  7. Client `tsc -b` — clean.
  8. Server + client `eslint` — 0 errors (client: 2 pre-existing unrelated `react-refresh` warnings).
  9. Full builds — server `prisma generate && tsc` clean; client `tsc -b && vite build` succeeds (pre-existing >500kB chunk warning, unrelated).
  10. `git diff --check` — exit 0, only pre-existing LF/CRLF autocrlf advisories, no whitespace-error/conflict-marker regressions.
  11. Documentation reconciliation: `docs/17-decisions-log.md` (new **ADR-057** + amendments to ADR-052 and ADR-045), `docs/21-email-integration.md`, `docs/20-whatsapp-integration.md`, `docs/23-sms-integration.md`, `docs/07-ticket-workflow.md`, `docs/22-realtime-events.md`, `docs/05-api-contract.md` (conversation `contentFormat`/`delivery`/`attachmentIds`/staged-upload endpoints), `docs/19-progress-tracking.md` (new dated current-state block) all updated. `docs/08-sla-automation.md` reviewed — no change needed (CONV-046 was confirmation-only, no behavior change). `docs/04-database-design.md` intentionally left untouched — it is an explicitly-labeled "planned logical model" historical artifact already stale relative to the real schema before this session, not in CONV-057's named doc list, and not a live contract.
  - Full client suite — **833+/835 passing, 68 files** (two parallel-load / machine-contention test timeouts — which specific files time out varies run to run, not a fixed file — individually rerun and verified passing; not a regression).

## Honest status at end of this session

**All 57 tasks (CONV-001–057) materially complete**, final gate (CONV-057) passed: server 1085/1085 tests (58 files), client 833+/835 tests (68 files, 2 confirmed-unrelated contention timeouts), both `tsc`/`eslint`/build clean, `git diff --check` clean, migrations additive-only and applied, docs reconciled (new ADR-057 + amendments + 7 doc files updated).

**One genuine, disclosed scope gap, not silently dropped:** CONV-049 (staged-attachment composer) was fully built for the **staff Ticket Details** surface (stage → chip → bind-on-send, channel-gated for EMAIL/SMS/WHATSAPP) and the **server-side contract** for Portal (`POST /portal/attachments/staged`, `portalReplySchema.attachmentIds`, atomic bind in `portal.service.reply`) is live and tested — but the **Portal/Live-Chat reply composer's client UI** was not given an Attach control this session (it has none today; adding one is a UI-only follow-up, not a data-model or API gap). Two related items were assessed as already-sufficient by inspection rather than duplicated with new test files: CONV-050's Live Chat widget session-key lifecycle (built in an earlier session, re-verified against the full CONV-050 text, no gap found) and CONV-054's "consolidation matrix" (the shared modules it would parameterize over — `resolveCorrelatedTicket`/`persistInboundMessage` — are deliberately channel-agnostic by design, so the channel-specific assertions genuinely live in, and are proven by, the existing per-channel test files; duplicating them into a second file was judged to add cost without new verification value — see the CONV-054 log entry above for the full reasoning and file references).

Given the above, the feature is marked below **`IMPLEMENTED + VERIFIED ON SDD BRANCH`**, with the Portal composer attach-UI named explicitly as the one remaining follow-up.
