...
| Spec status | [`spec.md`](./spec.md) — `READY FOR PLAN` |
| Plan status | [`plan.md`](./plan.md) — `READY FOR TASKS` |
...
- [x] **MS-06** — Edit-form Priority/Category select could revert to empty right after hydration *(client)* — regression found while smoke-testing the edit form after MS-05. No backend change.
  - **Symptom:** opening `/tickets/:id/edit` on an existing ticket sometimes showed Priority empty / Category empty (other fields hydrated fine).
  - **Investigation:** traced `GET /tickets/:id` → `ticket.data` → the MS-05 `hydratedFieldsRef` signature guard → `reset()` → RHF field values → `AppSelectField`. Instrumented the hydration effect, the `reset()` payload, and every Controller's `field.value` per render: the MS-05 guard fired **exactly once**, with the correct `d.priority` / `d.category?.id`, and RHF's own values stayed correct throughout — the guard was not the cause.
...
- [x] **MS-07** — `/tickets/:id/edit` Save was a silent no-op on every ticket *(client)*. No backend change.
  - **Symptom:** on the ticket Edit page, clicking Save did nothing — no error, no navigation, no request.
  - **Investigation (traced the full submit path):** the Save button is inside `<TicketFormShell onSubmit={submit}>` and is a real (default `type="submit"`) button — click correctly fires the form `submit` event → RHF `handleSubmit`. Instrumented `formState.errors` on every render: `handleSubmit` **did** run and **did** route to the invalid-branch every time, with `errors.channel` set (`"Invalid option: expected one of \"WEB\"|\"EMAIL\"|\"WHATSAPP\"|\"SMS\""`) — `submit(values)` / `update.mutateAsync` were **never reached**, so no PATCH was ever sent. `isSubmitting` never flipped true. No hidden error render existed for this because the Channel field (and its error slot) is wrapped in `{!editing && (...)}` — the failing field is invisible in edit mode, so the validation failure had **zero UI signal**. Department/Team/Agent routing values were confirmed valid at submit time (not the cause); the MS-06 RadixSelect raw-empty guard was confirmed unrelated (channel uses `register`/`Controller`-free plain state via `useWatch`, not the non-searchable `AppSelect`, and the field isn't even rendered on edit).
