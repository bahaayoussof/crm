...
| Ready for `plan.md`? | **Not produced in this pass, per instruction** (discovery-only phase). Given zero open decisions, Customers is procedurally ready for a `plan.md`/`tasks.md` pass whenever the developer wants one; this document intentionally stops short of proposing changes. |
...
**CUST-FOLLOWUP-001 (implemented, closes the residual race noted below):** a PostgreSQL functional unique index `Customer_email_lower_key` on `LOWER(email)` (migration `20260912163955_customer_email_lower_unique`) now makes the database the final authority for case-insensitive uniqueness. The plain `@unique` on `Customer.email` (exact-case) is left in place unchanged — the two constraints coexist, the functional one being the stricter superset. A dev-DB duplicate scan (`SELECT LOWER(email), COUNT(*) ... HAVING COUNT(*) > 1`) found zero existing case-variant duplicates before the migration was applied. Verified against the real dev database that a violation of the functional index still raises Prisma `P2002` (`meta.target: ["lower(email)"]`), so the existing `P2002` catch blocks in `createCustomer`/`updateCustomer` require **no code change** — they already map any `P2002` on this model to `409 CUSTOMER_EMAIL_EXISTS` regardless of which unique constraint fired. Regression coverage (`customer.test.ts`) simulates the race directly: the app-level `findFirst` pre-check is stubbed to miss (as it would in the real race window between two concurrent case-variant writes), and the underlying Prisma call is stubbed to reject with the same `P2002`/`lower(email)` shape confirmed against the real index — proving the DB constraint, not just the app pre-check, is what ultimately prevents two case-variant rows from ever coexisting, on both create and update. The residual risk below is now closed.
