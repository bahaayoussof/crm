...
- `POST /api/tickets/:id/messages` — unchanged authenticated conversation endpoint; an SMS-channel ticket dispatches plain text through the configured `SmsProvider` **after the reply transaction commits** (mirrors WhatsApp/Email — ADR-052). A missing phone, missing configuration, or provider failure now returns `201` with `delivery: { channel: "SMS", status: "FAILED", reason }` and an `SMS_DELIVERY_FAILED` ticket-history row; the local reply is always persisted. Auth / access / body validation still fail normally before anything is written.
...
**Session invalidation:** the JWT carries `iat`. Every role-protected API resolves the current user before authorization: deleted/deactivated accounts receive `401 ACCOUNT_DEACTIVATED`, a token issued before `passwordChangedAt` receives `401 SESSION_EXPIRED`, and the persisted role replaces the JWT role before the route allowlist and downstream visibility checks. `/auth/me` and `require-fresh-token` retain equivalent checks. Tokens remain stateless with an 8-hour expiry; there is no server-side session registry or refresh-token flow.
...
**AGENT self-assignment.** `PATCH /tickets/:id` with a body of exactly `{ "assignedAgentId": <self> }` lets an agent claim an **unassigned** ticket. It is applied atomically (`UPDATE … WHERE assignedToId IS NULL`): if the ticket was claimed by someone else between load and save the response is `409 TICKET_ALREADY_ASSIGNED`; if it was already the caller's it is an idempotent `200`. An agent supplying any other `assignedAgentId`, a `null` (release), or bundling `assignedAgentId` with other fields → `403 FORBIDDEN`. A ticket not visible to the agent → `404 TICKET_NOT_FOUND`. `ADMIN`/`MANAGER` assignment is unchanged.
  ...
  "conversation": [
    {
      ...
      "delivery": { "status": "FAILED", "reason": "PROVIDER_UNREACHABLE" },
...
**`contentFormat` / `delivery` (ADR-057).** Every item carries the persisted `contentFormat` (`"PLAIN_TEXT" | "SANITIZED_HTML"`) — the client renders from this field, never by sniffing `body` for markup. A `PUBLIC_MESSAGE` on a provider-backed channel (`EMAIL`/`SMS`/`WHATSAPP`) also carries `delivery` — `null` when no `MessageDelivery` row exists (WEB/LIVE_CHAT, or a row not yet created), otherwise `{ status, reason }` where `status` is the durable `PENDING | SENDING | SENT | DELIVERED | FAILED` and `reason` is the same coarse category as the send-response `delivery.reason` (never a provider id or raw error text). This reflects the row's *current* state — a later retry or provider callback updates it, so a reload after the initial `201` can show a different `status` than the send response did. `INTERNAL_NOTE` items never carry `delivery` (notes are never provider-delivered).
...
Both mutations accept the strict body `{ body: string, attachmentIds?: string[] }` (`body` min 1, max 50000 — markup headroom over the client's 20k plain-text limit; `attachmentIds` optional, max 10), trim `body`, reject empty or unknown fields, and derive `authorUserId` from the authenticated internal identity. They never accept a client-provided author or timestamp.

**Staged attachments (ADR-057).** `attachmentIds` names previously-staged, unbound attachments (`POST /attachments/staged`, see Attachments below) owned by the caller; they are validated and bound atomically inside the same transaction as the message/note create. For `POST /tickets/:id/messages` on an `EMAIL`/`SMS`/`WHATSAPP`-channel ticket, a non-empty `attachmentIds` is rejected with `422 ATTACHMENTS_NOT_SUPPORTED_FOR_CHANNEL` **before** any `TicketMessage` is created — never a silent text-only send. `POST /tickets/:id/notes` has no such restriction (notes are never provider-delivered).

`POST /tickets/:id/messages` creates a customer-visible `TicketMessage`. The reply body is **rich text (HTML) from the Lexical composer** and is **sanitized server-side on write** to a fixed support-reply allowlist (`b/strong/i/em/u/p/br/ul/ol/li/a[href]`; `http`/`https`/`mailto` only; links forced to `rel="noopener noreferrer nofollow" target="_blank"`; scripts, styles, classes, ids, event handlers, media, iframes, and data URIs discarded). A body that is empty once sanitized (markup-only) is rejected with `422 EMPTY_MESSAGE`. The stored value is trusted markup; every consumer that must not carry markup flattens it to plain text first — WhatsApp outbound delivery and the AI prompt context. Historical plain-text bodies are unaffected. The first successful public staff reply sets `firstRespondedAt` to the message creation timestamp in the same transaction only when it is currently null; later replies do not overwrite it. The response is `{ data: { kind: "PUBLIC_MESSAGE", id, body, createdAt, contentFormat, delivery, author } }` (`body` = the sanitized HTML; `contentFormat` — ADR-057 — is `"SANITIZED_HTML"` for a staff/Portal reply and `"PLAIN_TEXT"` for provider-delivered inbound bodies, and drives client rendering directly instead of the body being sniffed for markup).

**Outbound provider delivery is decoupled from local persistence for every provider-backed channel (ADR-052).** The `TicketMessage`, `firstRespondedAt` stamping and watcher notifications commit first; the realtime `ticket.message.created` event is published on commit; only then is the external provider attempted. A provider or configuration failure never rolls back the reply — it comes back as `delivery.status = "FAILED"` and is recorded as a `<CHANNEL>_DELIVERY_FAILED` ticket-history row. Auth, ticket access, agent-assignment, and body validation still fail normally (`401/403/404/422`) before any row is written.

For `WHATSAPP`, `EMAIL`, and `SMS` channels the response carries a `delivery` field: `{ data: { kind: "PUBLIC_MESSAGE", …, delivery: { channel, status: "SENT", externalId } } }` on success, or `{ …, delivery: { channel, status: "FAILED", reason } }` on failure, with `reason` ∈ `INTEGRATION_NOT_CONFIGURED | NO_RECIPIENT_PHONE | NO_RECIPIENT_EMAIL | RECIPIENT_INVALID | PROVIDER_REJECTED | PROVIDER_UNREACHABLE` (HTTP `201` in both cases). The outbound `EMAIL` (Resend) and `SMS` (TextBee) provider requests are each explicitly bounded to 20 seconds; an outbound timeout or a never-connected request is reported as `reason: "PROVIDER_UNREACHABLE"`, never `PROVIDER_REJECTED` (which stays reserved for a provider that responded and rejected the send). `WEB` / `LIVE_CHAT` tickets have no `delivery` field on send. **Durable delivery (ADR-057):** this `delivery` field is only the initial-attempt confirmation for the synchronous caller — the durable state lives in a `MessageDelivery` row and is what the ticket-detail `GET` (below) reflects on reload, including after a later retry or provider callback. `TicketMessage.externalId`/`providerMessageId` are never included in either shape — only the coarse `reason` category. Internal notes never send. See "WhatsApp Integration" below, `docs/20-whatsapp-integration.md`, and `docs/21-email-integration.md`.
...
POST /api/attachments/staged                    (ADR-057 — staff)
POST /api/portal/attachments/staged              (ADR-057 — Portal)
```

**Staged uploads (ADR-057).** `POST /api/attachments/staged` / `POST /api/portal/attachments/staged` accept the same single-`file` multipart upload as the routes above, but create an **unbound** row owned by the authenticated caller (`stagedByUserId`) — no `ticketId`/`messageId`/`noteId`/`customerId` yet. The response shape matches the internal/Portal projection below. The composer submits the returned `id` as one of `attachmentIds` on the following `POST /tickets/:id/messages`, `POST /tickets/:id/notes`, or `POST /portal/tickets/:id/messages` — that request atomically validates ownership and still-unbound state and binds the row to the exact created message/note in the same transaction (a mismatch → `403 FORBIDDEN`; an unknown id → `404 ATTACHMENT_NOT_FOUND`). The pre-existing ticket-level/message-level/customer-level upload routes above are unchanged and remain for their non-conversation uses (general ticket files, customer-profile files) — they are never themselves treated as a conversation attachment binding.
...
**Metadata projections.** Internal: `{ id, fileName, mimeType, createdAt, ticketId, messageId, noteId, customerId }` (`noteId` added — ADR-057). Portal (distinct serializer): `{ id, fileName, mimeType, createdAt, messageId }` only — no `ticketId`, `noteId`, `customerId`, `storageKey`, provider URL, or staff identity; note-owned attachments are internal-only and never appear in a Portal response. `GET /api/tickets/:ticketId/attachments` returns ticket-level, message-level, and note-level rows for the ticket, each once (`OR: [{ ticketId, messageId: null, noteId: null }, { message: { ticketId } }, { note: { ticketId } }]`); the focused message route returns that message's rows only.
...
`GET /dashboard/overview` requires `ADMIN`, `MANAGER`, or `AGENT`; `CUSTOMER` is rejected. `ADMIN` and `MANAGER` receive metrics and tickets across all internal tickets (unchanged). For an **`AGENT` the dashboard is a personal work console**: every field — `metrics`, `statusDistribution`, `ticketActivity`, `recentTickets`, and the primary queue — is scoped to `assignedToId === self` only (NOT assigned-or-unassigned, NOT organization-wide). The AGENT response additionally carries an `agentPerformance` object (absent for `ADMIN`/`MANAGER`): `{ windowDays, avgFirstResponseMinutes, avgResolutionMinutes, resolvedCount, slaCompliancePct, csat: { averageRating, responseCount } }` over a trailing 30-day window, all derived from that agent's own tickets and their feedback. No agent ranking, team comparison, or org-wide SLA/analytics is exposed to `AGENT` — those stay in the `ADMIN`/`MANAGER`-only `/reports` endpoints.

The standard `{ data }` envelope contains `metrics`, real counts grouped in `statusDistribution`, `ticketActivity`, `primaryQueueType`, at most 10 `primaryTickets`, at most 8 `recentTickets`, `agentPerformance` (AGENT only), and `generatedAt`. `primaryQueueType` is `NEEDS_ATTENTION` for `ADMIN`/`MANAGER` and `MY_ASSIGNED_TICKETS` for `AGENT`. Dashboard ticket items expose only identifier, subject, status, priority, update time, effective SLA deadline/state, safe customer summary, and safe assignee summary.

`ticketActivity` is a fixed-length array of exactly 30 `{ date, opened, resolved }` objects — one per UTC day, oldest first, ending on the current UTC day, zero-filled. `opened` counts tickets whose `createdAt` falls on that day; `resolved` counts tickets whose `resolvedAt` falls on that day. The series honours the same role scope as every other field (ADMIN/MANAGER: all internal tickets; AGENT: assigned-to-self only). It is an additive presentation aid for the dashboard "Ticket activity" chart and is independent of the ADMIN/MANAGER-only `/reports` endpoints. See `17-decisions-log.md` ADR-031.
...
## Manager Work Console — LIVE (on `feature/manager-work-console`, not yet integrated; ADR-047)

```text
GET /manager/overview
GET /manager/team
GET /manager/team/:agentId
```

All three require `ADMIN` or `MANAGER` (`requireAuth` + `requireRole(ADMIN, MANAGER)`); `AGENT`, `CUSTOMER`, and unauthenticated callers get `403`/`401`. Read-only aggregation — **no mutations**. All aggregates are **organization-wide**: there is no `Manager → Department/Team` relation in the schema, so `managerTicketScopeWhere` is an empty predicate and `GET /manager/overview` reports `meta.visibility = "ORGANIZATION_WIDE"`. SLA math reuses `shared/sla/sla-outcomes.ts` and `derive-sla.ts` — the same primitives as `/reports`; no calculation is duplicated.

- `GET /manager/overview` — one batched response: `needsAttention` (array of `{ key, count, ticketFilter }` for `slaBreached` / `slaAtRisk` / `escalated` / `unassignedUrgent`, where `ticketFilter` is a `/tickets` query string), `kpis` (`openTickets`, `unassigned`, `resolvedToday`, `slaCompliancePct`, `avgFirstResponseMinutes`, `avgResolutionMinutes` — the last three over a trailing 30 days), `teamWorkload` (**every active agent**, sorted by `openAssigned` desc: `{ agentId, agentName, openAssigned, inProgress, waitingCustomer, atRisk, resolvedToday }`), `priorityWork` (≤10 escalated/breached/urgent tickets, same safe projection as the dashboard: id, subject, status, priority, updatedAt, effectiveSlaDueAt, slaState, customer, assignedAgent), `generatedAt`.
- `GET /manager/team` — full agent table. Query: `page` (≥1, default 1), `limit` (1–100, default 15), `search` (≤100, name contains), `sortBy` (`name` | `openAssigned` | `inProgress` | `waitingCustomer` | `resolved` | `slaCompliance` | `avgFirstResponse` | `avgResolution`), `sortOrder` (`asc` | `desc`, default `desc`). Rows add trailing-30-day `resolved`, `slaCompliancePct`, `avgFirstResponseMinutes`, `avgResolutionMinutes` to the live workload counts. `{ data, pagination: { page, limit, total, totalPages }, meta, generatedAt }`. Search/sort/pagination are applied in memory (small roster).
- `GET /manager/team/:agentId` — `agentId` must be a CUID for an `AGENT` user, else `404 AGENT_NOT_FOUND` (a CUSTOMER/ADMIN id also 404s). Returns `{ agent: { id, name, email }, workload: { openAssigned, inProgress, waitingCustomer, escalated }, slaRisk: { breached, atRisk }, performance: { windowDays, avgFirstResponseMinutes, avgResolutionMinutes, resolvedCount, slaCompliancePct, csat: { averageRating, responseCount } }, atRiskTickets: [≤8], recentTickets: [≤8], meta, generatedAt }`.
...
# Audit Logs — LIVE on `feature/audit-logs`

`GET /api/audit-logs` is `ADMIN` only (`MANAGER`/`AGENT`/`CUSTOMER` → `403`, anonymous → `401`). Strict query parameters: `page`, `limit` (max 100), `search`, `actorId`, `action`, `entityType`, `entityId`, `from`, `to`; unknown keys and `from > to` are rejected. Results use the standard `{ data, meta }` pagination envelope and stable `createdAt DESC, id DESC` ordering. Search covers action, entity type/id, and safe actor name/email only.

Each record returns `id`, string `action`, string `entityType`, nullable `entityId`, safe actor `{id,name,email}` or `null` for system activity, allowlisted `changes`, safe `metadata`, nullable IP/User-Agent, and `createdAt`. Passwords, hashes, tokens, credentials, authorization/cookie headers, request bodies, message/note bodies, and arbitrary DTO/record serialization are forbidden.
