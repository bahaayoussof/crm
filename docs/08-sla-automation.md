...
Both actions use conditional updates inside transactions. History and notifications are inserted only after the guarded update affects exactly one row. Repeated or overlapping executions therefore create no duplicate mutation, history, or notification for unchanged state. Automated history has `actorUserId: null`; assignment alerts target the selected agent. Escalation alerts use the same team-scoped operational-recipient rule as the manual `ESCALATED` transition (`ticketOperationalRecipientIds`, shared with `ticket.service.ts`): every active `ADMIN`, plus **only the manager of the escalated ticket's owning team** (`Ticket.teamId`), plus the ticket's own assigned agent when it has one; an unrouted, unassigned ticket alerts active `ADMIN` users only, never every manager. This is the team-scoped recipient rule documented in `docs/06-auth-rbac.md` and `specs/domain-model.md` — the SLA monitor does not broadcast to all `MANAGER` users. No `isBreached`, `slaStatus`, or other derived state is stored.

Each candidate (assignment or escalation) runs its own transaction and is processed independently: a single candidate's transaction failure (for example a Neon interactive-transaction timeout under real latency, a known risk for this codebase's transactional writes) is caught and logged, and the batch continues with the remaining candidates rather than aborting the whole run. Every earlier candidate in the same run has already committed, so aborting on one failure would only silently drop their buffered realtime events without undoing any persisted state. A failed candidate is simply retried by the next scheduled run — there is no dead-letter queue or backoff; this mirrors the project's "no generic job queue" constraint.
