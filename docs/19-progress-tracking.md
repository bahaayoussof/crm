...
Last Updated: 2026-08-26 (feature/attachments — backend + strict multipart-field rejection + icon Preview/Download UX; implemented, not integrated)
...
Current Working Branch: `feature/attachments`, created from clean synchronized `master` at `069839a` (`docs: sync knowledge base integration status`, a docs-only commit on top of `ef647ef`; `master` == `origin/master`). Secure Attachments (backend upload/download/authorization + strict multipart-field rejection + an icon-only Download button and an accessible in-browser Preview dialog) is implemented on this branch and **not integrated into `master`**. Nothing has been staged, committed, pushed, merged, rebased, amended, or tagged. Working-tree changes beyond source: `server/package.json`/`package-lock.json` gain `@vercel/blob` + `busboy` + `@types/busboy`; `.gitignore` gains `.vercel` (an over-broad `.env*` line that the Vercel CLI added, which re-ignored every `.env.example`, was removed); developer-provided `server/.env` + `server/.env.local` (git-ignored) now carry a real private-Blob `BLOB_READ_WRITE_TOKEN`. Git: 26 modified tracked files, 26 new files (14 `server/src/modules/attachments/*`, 12 `client/src/features/attachments/*`).
