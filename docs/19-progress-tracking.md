...
Last Updated: 2026-08-27 (`feature/quick-replies` integrated into `master` at `79c7067 feat: implement quick replies`; `feature/customer-feedback` — roadmap order 4 — implemented on its branch off `79c7067`, NOT integrated: Portal-only `POST`/`GET /api/portal/tickets/:id/feedback`, one immutable rating (1–5) + optional comment per own `RESOLVED`/`CLOSED` ticket, `feedbackEligible`/`feedback` folded into the Portal ticket-detail payload, star-rating Portal UI, EN/AR + RTL, no schema change, ADR-023. Same branch also carries a pre-integration **Customer Portal Ticket Details design-alignment** pass: extracted role-neutral `client/src/features/tickets/ticket-conversation-ui.tsx` primitives now shared by the internal and Portal ticket views, Portal conversation/description/composer/status-badge/attachments rebuilt to the internal visual language, message authors relabelled "You"/"Support Team"; no new endpoint, no internal-API call from the Portal, no schema change; client 269 / server 253 / 522 total, 0 failed.)

Current Integration Branch: `master` at `79c7067` (`79c7067b9d48a86058211cf9d0865668aa157056`), equal to the `origin/master` tracking ref (ahead 0, behind 0). `79c7067 feat: implement quick replies` sits on `e34818b docs: sync attachments integration status` → `cb63871 update tracking file` → `8e24d22 feat: implement secure attachments` → `069839a docs: sync knowledge base integration status` → `ef647ef feat: implement knowledge base` → `d89cf47` (docs-only) → `e387667` (docs-only) → `e7d9b14 feat: complete basic SLA presentation`. `feature/sla` (`e7d9b14`), `feature/knowledge-base` (`ef647ef`), `feature/attachments` (`8e24d22`), and `feature/quick-replies` (`79c7067`) are all contained in `master`; each was integrated by fast-forward with no merge commit.

Current Working Branch: `feature/customer-feedback`, branched from `master` at `79c7067` (working tree clean before this feature; `master` synchronized with `origin/master` at task start). Customer Feedback — Portal-only `POST /api/portal/tickets/:id/feedback` and `GET /api/portal/tickets/:id/feedback` as `portalRouter` sub-routes (`requireRole(CUSTOMER)`), one immutable `Feedback` row per owned `RESOLVED`/`CLOSED` ticket (`rating` int 1–5, optional `comment` ≤2,000, `customerId` server-derived, `409 TICKET_NOT_ELIGIBLE_FOR_FEEDBACK` / `409 FEEDBACK_ALREADY_SUBMITTED` / IDOR-safe `404 TICKET_NOT_FOUND`), one `TicketHistory` `FEEDBACK_SUBMITTED` row per submission, `feedbackEligible` + `feedback` added to the Portal ticket-detail response, a star-rating + optional-comment Portal section with a read-only submitted state; English/Arabic and RTL; **no Prisma schema or migration change**; ADR-023 — is implemented on the uncommitted `feature/customer-feedback` branch and is NOT integrated. It stages, commits, pushes, merges, rebases, amends, and tags nothing.
...
| Users Management | 🟢 IMPLEMENTED — on branch, not integrated | `feature/user-management` (roadmap order 6), uncommitted | `server/src/modules/users/*`: `GET /users`, `GET /users/:id`, `POST /users`, `PATCH /users/:id` (name/email/role/isActive — **single safe update path**, only submitted keys written, runs in one `$transaction`) — all `requireAuth` → `requireActiveUser` → `requireRole(ADMIN)` (no `app.ts` change; `/users/agents` keeps its ADMIN/MANAGER/AGENT lookup + filters `isActive`). No `PATCH /users/:id/role` route. Strict Zod; safe projection (no `passwordHash`); bcrypt cost 12; internal-role-only (`CUSTOMER` id → 404). Conflicts: self role change → 409 `SELF_ROLE_CHANGE_FORBIDDEN`, self deactivation → 409 `SELF_DEACTIVATION_FORBIDDEN`, last active ADMIN demote/deactivate → 409 `LAST_ACTIVE_ADMIN_REQUIRED`, dup email → 409 `EMAIL_ALREADY_REGISTERED`. New `server/src/middleware/require-active-user.ts` resolves the caller's current DB role/active state before `requireRole` (demoted → 403, deactivated → 401 `ACCOUNT_DEACTIVATED`) — scoped to `/api/users` admin routes. Schema: `User.isActive Boolean @default(true)` (migration `20260827101406_add_user_is_active`). `auth.service.ts`: deactivated login → 403, `/auth/me` mid-session → 401 `ACCOUNT_DEACTIVATED` | `client/src/features/users/*` + `client/src/app/router/user-manage-route.tsx` (non-ADMIN → `/dashboard`); `/users` list (search + role + status filters synced to URL, `table-fixed` TanStack table with read-only Role/Status **badges** + `truncate` email + `You` badge + mobile cards), `/users/new` + `/users/:id/edit` forms (Role `<select>` in Edit, disabled + explained on your own account; Active checkbox disabled for self), row Edit + Deactivate/Reactivate actions; the confirmation is a **portalled** anchored `role="dialog"` (`aria-modal`) rendered on `document.body` via the shared `components/shared/use-anchored-popover.ts` primitive (`position: fixed`, pinned to the trigger's logical end, viewport-clamped incl. 320 px, flips above, `z-50`) — it no longer lives in the table's `overflow-x-auto` wrapper, so opening it adds no scrollbar and shifts no table content; single-open (stable user id), switches on another trigger, closes on filter/pagination, Escape, outside pointer; focus → Cancel on open, → trigger on close, Tab trapped; `NativeSelect` primitive (one custom chevron, LTR/RTL, `appearance-none`); conditional nav item (`canManageUsers` = ADMIN); `users.*` + `navigation.users` EN/AR (644/644 parity), RTL | server `user.test.ts` 31 + `auth.test.ts` 2 deactivation + `middleware/auth.test.ts`; client `users.test.tsx` 35 (incl. portal-outside-table, one-open/switch, Escape/outside close, focus in/return, stale-close on filter, stable-id bind, + `computeAnchoredPosition` unit: below/flip/clamp/RTL/320px) + `user-manage-route.test.tsx` 4; full suites on branch: **client 321 / server 298**, 0 failed / 0 skipped / 0 todo | Not PostgreSQL/browser verified; deterministic mocked tests cover every route/role/guard/validation/self-protection/last-admin/stale-JWT-role path and the portalled-popover placement/interaction | ADR-025 (+ pre-integration correction addendum; portalled status-confirmation correction — no new ADR). One column added (`User.isActive`). Limitations: no password-reset flow, no bulk actions, no CSV import/export, no admin-action audit log, no MFA/session revocation, no department/branch assignment, no user-deletion route (retire via `isActive=false`); active-session freshness is enforced on `/api/users` + `/auth/me` only (other routers use JWT role until 8h expiry — deliberate, no per-request lookup, no refresh tokens). Historical Vite chunk-size warning remains non-failing. |
...
`feature/customer-feedback` is integrated into `master` at `12a0c12`; `feature/reports` (roadmap order 5) is integrated into `master` at `827b3ff feat: implement reports`. `feature/user-management` (roadmap order 6) is implemented on its uncommitted branch and automated-verified (client 321 / server 298, 0 failed) — ADMIN-only `/api/users` CRUD with role changes folded into `PATCH /users/:id`, `User.isActive` column + `ACCOUNT_DEACTIVATED` enforcement, `requireActiveUser` DB-fresh role check, server-enforced self-role / self-deactivation / last-active-admin guards, and a `/users` admin page (read-only role/status badges, Edit-form role select, portalled anchored deactivate/reactivate confirmation via the shared `use-anchored-popover` primitive) — ADR-025 plus pre-integration correction addenda. Next implementation feature after developer integration: **`feature/settings`** (roadmap order 7). Final demo seed data is still not next (section 6A).
