...
### Known limitation — Manager → Department/Team ownership

There is no schema relation that ties a `MANAGER` to a department, branch, or team (`Department` and `Branch` have no `managerId`; `User.departmentId`/`branchId` describe the user's own placement, not what they manage). Until such a relation exists, every Manager sees organization-wide tickets, agents, reports, tasks, and SLA metrics — the same scope MANAGER had before the Manager Work Console. When the relation is added, scoping is applied in exactly one place per resource: `managerTicketScopeWhere(actor)` and `managerAgentWhere(actor)` in `server/src/modules/manager/manager.service.ts`.
