...
- Users administration (`GET /api/users`, `GET /api/users/:id`, `POST /api/users`, `PATCH /api/users/:id`, `feature/user-management`): **`ADMIN` only** (`requireAuth` → `requireActiveUser` → `requireRole(ADMIN)` per route; `/users/agents` keeps its `ADMIN`/`MANAGER`/`AGENT` lookup group). `MANAGER`, `AGENT`, `CUSTOMER`, and unauthenticated callers receive `403 FORBIDDEN` / `401` — the `docs/18` §15 "MANAGER only if explicitly granted" default resolves to **not granted**. Acts on internal identities only; a `CUSTOMER` id is `404`. `role` is constrained to `{ADMIN,MANAGER,AGENT}` on create and update.
  ...
  - **Active-session enforcement:** the shared `requireRole` guard reads the caller's current `role`, `isActive`, and `passwordChangedAt` before every role-protected API authorization. Demotion/deactivation/password change therefore takes effect on the next request. A deleted/deactivated caller gets `401 ACCOUNT_DEACTIVATED`; a stale pre-password-change token gets `401 SESSION_EXPIRED`; a current but disallowed role gets `403 FORBIDDEN`. `requireActiveUser` remains on the user/settings admin surfaces as defense in depth. There is still no refresh-token or server-side session registry.
...
- Manager Work Console (`feature/manager-work-console`, ADR-049): read-only aggregation endpoints for the dedicated Manager operations experience. `GET /api/manager/overview`, `GET /api/manager/team`, `GET /api/manager/team/:agentId` are `requireAuth` + `requireRole(ADMIN, MANAGER)`; `AGENT`, `CUSTOMER`, and unauthenticated callers receive `403`/`401`. They add **no mutation capability** — every write the console UI triggers (assign, reassign, escalate, reply, note, status/priority) goes through the existing ticket endpoints with their existing RBAC. **Manager data scope is organization-wide**: the schema has no `Manager -> Department/Team` ownership relation, so `managerTicketScopeWhere` is an empty predicate today and the overview response reports `meta.visibility = "ORGANIZATION_WIDE"`. The additive `GET /api/tickets` filters `sla=breached|at_risk` and `assignee=unassigned` are available to `ADMIN`/`MANAGER` only and never widen an `AGENT` list. The client hides Customers and Quick Replies from the MANAGER navigation (routes and backend RBAC unchanged); `ManagerRoute` admits `MANAGER` and `ADMIN` and redirects everyone else to `/dashboard`.

### Team-based MANAGER scope (`feature/team-based-manager-scope`, ADR-050) — supersedes the ADR-049 "organization-wide MANAGER" statements above

A real `Team` model now ties a `MANAGER` to exactly one team (`Team.managerId`, `@unique`). **A MANAGER's authorization scope is their own team; it is no longer organization-wide.**

- **Single source of truth:** `server/src/shared/team/team-scope.ts` — `resolveActorTeamId(actor)` (one query: MANAGER → led team, AGENT → `User.teamId`, ADMIN → `null`), `teamScopedTicketWhere` / `teamScopedAgentWhere` (MANAGER → `{ teamId }`, or match-nothing when the manager has no team), `assertManagerTicketAccess` (404, no existence leak), `assertAgentAssignableToTicket` (409 `CROSS_TEAM_ASSIGNMENT` / `AGENT_HAS_NO_TEAM`), `ticketOperationalRecipientIds` (notification routing). No `if (role === MANAGER)` team checks are duplicated in individual modules.
- **Enforced server-side for MANAGER, own-team only:** `GET /api/tickets` (list) and `GET /api/tickets/:id` (detail — 404 for another team's id); ticket update / assignment / conversation / watchers; ticket attachments (list, upload, download-by-id); AI ticket actions; tasks (link check + a MANAGER's task list is own/assigned/unlinked + own-team ticket-linked tasks only); `GET /api/manager/{overview,team,team/:agentId}` (agent-detail 404 for an out-of-team agent; `meta.visibility = "TEAM"`, `meta.teamName`); Dashboard aggregates; every Reports endpoint (`overview`/`tickets`/`agents`/`sla` — the controller injects the manager's team id, or a match-nothing sentinel when they have no team); `GET /api/users/agents` (MANAGER always receives only their own team's agents and a client `teamId` is ignored). A **MANAGER with no team matches nothing** everywhere — there is no organization-wide fallback.
- **AGENT** keeps the existing model plus team narrowing: "mine" = own assigned tickets (anywhere); "unassigned" list + the single-ticket boundary + self-claim are restricted to unassigned tickets **within the agent's own team**. No agent permission is broadened.
- **ADMIN** is unchanged — organization-wide, no team predicate anywhere.
- **Assignment:** a ticket may only be assigned to an agent on the same team (`409 CROSS_TEAM_ASSIGNMENT`); an agent with no team is rejected (`409 AGENT_HAS_NO_TEAM`); an unrouted ticket (`teamId = null`) adopts the assignee's team; a ticket that already belongs to a team is never silently moved.
- **User Management (ADMIN):** create/edit accept `teamId`. For a MANAGER, assigning a team also sets `Team.managerId` (and frees the previous team); `409 TEAM_ALREADY_HAS_MANAGER` when the team already has a different manager. Moving an AGENT who still has active tickets on their current team is rejected with `409 AGENT_HAS_ACTIVE_TICKETS` (tickets are never migrated automatically).
- **Notifications & realtime:** operational ticket notifications (escalation, SLA breach, customer reply, inbound email/WhatsApp) go to every active ADMIN + **only** the ticket-team's manager + the assignee; an **unrouted ticket notifies ADMINs only — never every manager**. The realtime SSE `canReceive` is team-aware: a MANAGER receives ticket events only for their own team, an AGENT's unassigned-queue events are narrowed to their team, ADMIN is unchanged, CUSTOMER is unchanged.
...
- **Customer Management support-history list (`GET /api/customers/:id/tickets`, `listCustomerTickets`) — team-scoped for MANAGER (OD-6, resolves the former ADR-050 deferred exception).** It applies the canonical `teamScopedTicketWhere(actor, resolveActorTeamId(actor))` predicate: `ADMIN` org-wide, **`MANAGER` limited to their managed team** (teamless MANAGER → empty page, no org-wide fallback), `AGENT` still receives the customer's full history with `access = SUMMARY_ONLY` for another agent's ticket (the ADR-014 cross-agent support-history design). It grants no ticket detail / conversation / mutation route and is never a Ticket-visibility bypass. AGENT (full history, summary-only) and MANAGER (team-only) deliberately differ.
...
- **`AuditLog`** is the general-purpose, cross-entity administrative trail: one row per significant create / update / activate / deactivate / delete across `USER`, `CUSTOMER`, `TICKET`, `CATEGORY`, `SLA_RULE`, `DEPARTMENT`, `BRANCH`, `TEAM`, and `KNOWLEDGE_ARTICLE` (see `AUDIT_ACTIONS` / `AUDIT_ENTITY_TYPES` in `server/src/modules/audit-logs/`). Rows carry a nullable `actorId` (null with `metadata.actorType = "SYSTEM"` for cron-driven changes), `action`, `entityType`, `entityId`, a structured `metadata.changes` (`from` / `to` per field), and best-effort request `ipAddress` / `userAgent`. Ticket changes are recorded in *both* trails — e.g. an assignment writes a `TicketHistory` row and an `AuditLog` `TICKET_ASSIGNED` row. Audited ticket actions: `TICKET_CREATED`, `TICKET_STATUS_CHANGED`, `TICKET_PRIORITY_CHANGED`, `TICKET_CATEGORY_CHANGED`, `TICKET_ASSIGNED`, `TICKET_ESCALATED`, `TICKET_CLOSED`, and **`TICKET_ROUTING_CHANGED`** (OD-2) — one row per `PATCH /tickets/:id` that changes `departmentId` / `branchId` / `teamId` (including implicit team adoption), `metadata.changes` holding id values only, written in the update transaction; no row on a no-op or a rejected update, and no `TicketHistory` row for routing.
...
- Dashboard AGENT metrics, status distribution, and Recent Tickets are scoped strictly to that agent's assigned tickets (`assignedAgentId = self`); only the separate `unassignedTickets` counter reflects team-visible unassigned tickets, matching the same scope Ticket Management already grants that agent. See `specs/features/dashboard-reporting/spec.md`. This does not change Ticket Management access or add claiming.
...
Knowledge Base management mutations (create, edit, publish, unpublish, delete) now write one `AuditLog` row each with `entityType: KNOWLEDGE_ARTICLE`, transactionally with the mutation and with a server-derived `actorId`; reads and rejected mutations write nothing, and the article body is never placed in the audit metadata. This adds no permission change — the matrix above is unchanged.

Article authoring uses a bounded Lexical rich-text editor and the body is stored as server-sanitized HTML (ADR-057, `KB-RICH-*`); this is a UX/representation change only and does not alter any permission — the matrix above is unchanged.
