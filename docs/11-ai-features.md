...
`client/src/features/ai-assistant/` — `useTicketAiSummary` and `useTicketAiSuggestedReply` (both `useMutation`; result in mutation state only, never the ticket query cache) drive the internal **AI Assistant**. On-demand only: no AI request on page mount, and none when the workspace opens.

**Presentation — responsive drawer (not inline).** The Ticket Details sidebar shows only a compact launcher (**Open AI Assistant** + a short description, or an "_N_ results ready" count once something is generated). The interactive workspace opens in a shared `Sheet` primitive (`client/src/components/ui/sheet.tsx`): a right-side drawer at `lg`+ (`w-440px`, viewport-height, own internal scroll), a bottom sheet below `lg` (`max-h-[85dvh]`, rounded top, own internal scroll). It is a portalled `role="dialog"` overlay taken out of page flow, so generating long Summary / Reply / Category / KB output never grows the Ticket page or moves its scroll position, and it never auto-scrolls to a result. The desktop drawer stays on the physical right in RTL by product decision. `AiAssistantPanel` stays mounted (inside the always-mounted `TicketSidebar`) and owns all four AI mutations plus the open state, so closing and reopening the drawer preserves every generated result with no re-request — no global store. Close via the header **Close AI Assistant** button or `Escape`; focus returns to the launcher.

**Action grid.** The four capabilities are a 2×2 responsive card grid at the top of the drawer (`grid-cols-1 gap-3 sm:grid-cols-2`) — each card (`ai-action-card.tsx`, a real `<button>`) is icon + title + one-line description (`aiAssistant.actions.{summary,reply,category,solution}.description`, description `aria-hidden` so the accessible name is just the title). The card is the idle trigger and also carries the pending state: same size, `disabled`, spinner + the loading phrase as its label (and accessible name), so no duplicate request can fire. Results/errors render in the sections below the grid; the idle/pending sub-component branches render nothing.

- **Summarize Ticket** — idle/pending on the card → structured summary (Issue / Timeline / Current State / Recommended Next Action) + **Regenerate** → error (`role="alert"` mapped message + **Retry**).
- **Suggest Reply** — idle/pending on the card → draft in a read-only block + **Insert into Reply** / **Regenerate** → error + **Retry**. The two actions are independent (a reply error never clears a rendered summary). Regenerate re-runs the action only; it never touches the composer, mutates the ticket, persists, or notifies.
