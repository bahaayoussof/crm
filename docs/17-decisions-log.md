...
> **Superseded for assignment (ADR-051 + branch `fix/sla-team-scoped-assignment`, SEC/WF-1).** The department/branch assignment policy described in the paragraph above no longer exists. The cron now only *finds* candidates (unassigned + active status + routed `teamId`) and delegates the decision to the canonical team-scoped engine `autoAssignTicket(tx, …)`. `Ticket.teamId` is the authoritative ownership boundary; `teamId = null` tickets are left for ADMIN routing. The escalation behavior in this ADR is unchanged.
...
**2026-09-13 Auth/RBAC SDD amendment:** `requireRole` now resolves current `id`/`role`/`isActive`/`passwordChangedAt` for every role-protected API, replaces `request.auth.role`, and rejects deleted/deactivated or pre-password-change sessions before checking the route allowlist. This closes the stale-role privilege window across tickets, customers, reports, tasks, notifications, realtime, Portal, and nested resources while retaining stateless 8-hour JWTs and all domain-level visibility helpers. `requireActiveUser` and `requireFreshToken` remain as defense-in-depth on their existing routes. No refresh tokens or session registry were introduced.
...
- Every role can self-serve a password reset; links expire and are one-time; a reset/change immediately invalidates other sessions on `/auth/me`, Portal, and every role-protected API.
...
**Amendment (2026-09-13, Conversations/Channels — see ADR-057).** Two Email/WhatsApp inbound-reply emissions were found omitting `teamId` on an already-routed ticket (silently defaulting to `null` = ADMIN-only audience, so the assigned agent/team manager never got the invalidation signal) — fixed at the call site in both `email.service.ts` and `whatsapp.service.ts`; every `emitTicketMessageCreated`/`emitTicketUpdated` call site across all six ticket-creation channels now passes `teamId` explicitly. Durable outbound delivery (ADR-052 amendment) reuses the existing `ticket.updated` event for a genuine post-commit delivery-state change (retry success/failure, a supported provider callback) — **no new SSE event type was added**; a retry attempt or callback that leaves state unchanged (lost claim, stale/duplicate callback) emits nothing.
...
4. `reason` vocabulary (shared): `INTEGRATION_NOT_CONFIGURED | NO_RECIPIENT_PHONE | NO_RECIPIENT_EMAIL | RECIPIENT_INVALID | PROVIDER_REJECTED | PROVIDER_UNREACHABLE`. A raw network throw / timeout maps to `PROVIDER_UNREACHABLE`; the DB transaction is already closed so a hanging provider call cannot hold it open.
   - **Explicit outbound bound (follow-up).** The Resend send is now wrapped in `AbortSignal.timeout(EMAIL_DELIVERY_TIMEOUT_MS)` (`20_000` ms, matching the SMS `AbortSignal.timeout(20_000)`; deterministic constant in `email.client.ts`, not env-configured). The installed Resend SDK (6.25.0) does not type `signal` on its request options, but its runtime client spreads the options object into the underlying `fetch`, so the signal performs a **real request cancellation**, not a `Promise.race` that leaks the socket. `sendTicketEmail` raises `ResendEmailError("timeout")` when the request aborts (whether the SDK throws or swallows the abort into `result.error`); `deliverEmailReply` maps that to `AppError(504, "EMAIL_DELIVERY_TIMEOUT")`, and `outboundFailureReason` maps `EMAIL_DELIVERY_TIMEOUT` → `PROVIDER_UNREACHABLE` — never `PROVIDER_REJECTED`. The timeout fires only after the local `TicketMessage` (and its side effects + realtime event) is committed, so it cannot roll back the CRM reply, create a duplicate message, or emit a duplicate event.
   - **SMS timeout classification (follow-up).** `textBeeProvider.sendMessage` wraps only the `fetch` call in a pre-response `try/catch` — this is the "no HTTP response was received" seam. It previously collapsed every throw there into `AppError(502, "SMS_DELIVERY_FAILED")`, so an `AbortSignal.timeout(20_000)` firing was misreported as `PROVIDER_REJECTED`. It now throws `AppError(504, "SMS_DELIVERY_UNREACHABLE")` for both the timeout (`DOMException` "TimeoutError") and a raw connection/DNS failure (`TypeError: fetch failed`); `outboundFailureReason` maps `SMS_DELIVERY_UNREACHABLE` → `PROVIDER_UNREACHABLE`, consistent with Email. A TextBee that **responds** and rejects (non-2xx, `data.success === false`, `failureCount > 0`) still throws `SMS_DELIVERY_FAILED` → `PROVIDER_REJECTED`. No change to the 20 s bound, transaction boundary, or provider call ordering.
...
**Amendment (2026-09-13, `chore/sdd-foundation`, Conversations/Channels — see ADR-057).** This ADR remains the commit-first authority: the local `TicketMessage` and its transaction still commit before any provider call, unchanged. What changed is the *bookkeeping* of the post-commit attempt: `TicketMessage.externalId` is no longer written by any new outbound send (it stays populated only for historical/inbound rows). A new `MessageDelivery` row — one per outbound `TicketMessage`, `PENDING` at commit time — is the durable source of truth for `status`/`providerMessageId`/`attemptCount`/`lastErrorCode`, replacing the second best-effort `update` described in point 3 above. A bounded external-cron retry sweep (≤3 attempts, 1m then 5m) and, where a provider actually exposes one (Resend, Meta Cloud API), a delivery-status callback can now advance that same row post-commit. None of this reopens the transaction boundary or changes the `201` response shape for the synchronous caller.
...
**Status:** Accepted (implemented on branch `chore/sdd-foundation`, uncommitted). Decomposed and tracked in `specs/features/conversations-channels/{spec,plan,tasks}.md`; see CONV-001–057 for the full task-by-task record. This entry is the durable summary; the tasks file is the session working log.

## Context

An audit of the five conversation channels (Email, SMS, WhatsApp, Portal/Web, Live Chat) against the Tickets SDD's invariants found: two channels omitting `teamId` on inbound realtime events (silently ADMIN-only audience — see ADR-045 amendment above); SMS misclassifying an unrelated DB constraint violation as a duplicate-message drop; three channels selecting "the customer's newest active ticket" as a correlation fallback, which can silently append an unrelated reply to the wrong ticket or reopen a RESOLVED one; a Live Chat start race that could create two tickets for one session; ticket-creation audit logging present on some paths and absent on others; conversation body rendering decided by sniffing the body for markup instead of trusting server-declared provenance; and outbound provider delivery state tracked only as a best-effort field with no retry, no callback, and no durable per-message row.

## Decision

1. **Correlation precedence (OD-CC-4).** A shared, channel-agnostic `resolveCorrelatedTicket` seam (`server/src/modules/tickets/resolve-correlated-ticket.ts`) accepts only explicit ticket/session evidence or verified provider thread/message evidence — a customer id alone is never sufficient. Each channel keeps its own evidence extraction (Email RFC headers/reply token/subject reference; Live Chat session key; SMS/WhatsApp have no reliable thread signal today, so absent reliable evidence they always create a new ticket) and calls this seam with that evidence already extracted. The pre-existing "newest active ticket for this customer" fallback is removed from Email, SMS, and WhatsApp.
2. **RESOLVED reopen / CLOSED discard (OD-CC-3).** Only Portal (owned-ticket, explicit id) and reliably-correlated Email reopen a RESOLVED ticket. SMS, WhatsApp, and a new-session-key Live Chat never reopen — they create a fresh ticket. A CLOSED ticket is always immutable; a channel that would have appended to it instead discards the correlation and creates a new ticket (Email), excludes it at the query level (SMS/WhatsApp), or treats the session as non-resumable (Live Chat).
3. **Live Chat session-key concurrency (OD-CC-5, CC-GAP-22).** Start/resume is session-key-only (`Ticket.liveChatSessionKey`, unique). A create race resolves via the DB unique constraint: the losing request re-reads and returns the winner's ticket — never a second ticket, never a client-visible error shape difference.
4. **Canonical phone resolution (OD-CC-6).** `resolveCustomerByPhone` returns a `{none | one | ambiguous}` discriminated result; `ambiguous` performs zero writes and logs only a correlation id + count, never a phone number or candidate id, replacing SMS/WhatsApp's prior silent-newest-wins / warn-and-guess behavior.
5. **Audit consistency (OD-CC-7).** A shared `createCanonicalTicket` helper is the only path that creates a `Ticket` + its `TicketHistory(TICKET_CREATED)` + exactly one `AuditLog(TICKET_CREATED)` (actorless except staff/manual). All six creation paths (staff, Portal, Email, SMS, WhatsApp, Live Chat) route through it.
6. **Durable outbound delivery (OD-CC-1).** A `MessageDelivery` row (1:1 with an outbound `TicketMessage`, `PENDING` at commit) replaces the ADR-052 best-effort `externalId` update as the source of truth for delivery status. A bounded external-cron retry sweep (≤3 attempts, 1m then 5m, atomic claim-lease so overlapping invocations never double-send) and, where a provider actually exposes one (Resend `email.delivered`/`email.bounced`; Meta Cloud API `statuses`), a signed delivery-status callback can advance the same row post-commit. Neither ever creates a second `TicketMessage`/notification/audit row; a genuine state change emits one `ticket.updated` (no new SSE event type — ADR-045 amendment).
7. **Inbound idempotency (OD-CC-5).** Every inbound provider message gets a namespaced `TicketMessage.inboundKey` (`email:<id>`, `sms:<id>`, `whatsapp:<wamid>`), unique at the DB level. A shared `persistInboundMessage` helper classifies a duplicate **only** when the violated constraint targets `inboundKey` — an unrelated constraint violation (e.g. a placeholder-email collision) propagates as a real error instead of being silently dropped as a false duplicate (the SMS defect this closes).
8. **Content provenance (OD-CC-9).** Every conversation write declares `contentFormat` (`PLAIN_TEXT | SANITIZED_HTML`) and `contentSource` explicitly through one canonical validator (`shared/rich-text/conversation-content.ts`, 20,000-char default / 2,000 for Live Chat / 50,000 rich-payload ceiling). The client renders from this persisted field — the previous regex-based "does this look like HTML" sniffing is removed entirely, closing the defect where a plain-text body containing literal `<strong>` markup from SMS/WhatsApp/Email could render as formatted HTML.
9. **Attachment staging (OD-CC-2).** A conversation attachment is staged (owned by the uploading actor, unbound) via `POST /attachments/staged` (or the Portal equivalent), then bound atomically — inside the same transaction as the message/note create — to the exact `TicketMessage` or `TicketNote` it belongs to. EMAIL/SMS/WhatsApp outbound requests carrying a staged attachment are rejected (`422 ATTACHMENTS_NOT_SUPPORTED_FOR_CHANNEL`) before any message is created — never a silent text-only fallback; internal notes may always attach regardless of channel, since notes are never provider-delivered.

## Alternatives rejected

- **A generic outbox/job queue for retries.** Rejected — scope is one bounded, provider-agnostic retry sweep over a single `MessageDelivery` table; a queue is unjustified infrastructure for three provider channels with a 3-attempt cap.
- **Building delivery callbacks for every provider "for completeness."** Rejected — SMS/TextBee has no webhook infrastructure in this deployment; a fabricated endpoint would either never fire or silently do nothing. Only Email and WhatsApp, which already have a signed, configured, or account-supported callback contract, got one.
- **Reversing ADR-052's commit-first architecture to add delivery tracking.** Rejected — the commit-first guarantee is the load-bearing invariant; durable delivery is additive bookkeeping on top of it (see ADR-052 amendment above).

## Consequences

- Additive-only schema changes across two migrations: `ConversationContentFormat`/`ConversationContentSource` enums + columns, `MessageDelivery`, `TicketMessage.inboundKey`, `Attachment.noteId`/`stagedByUserId`, `Ticket.liveChatSessionKey` (all nullable at introduction; content-format columns made required only after a verified zero-null backfill).
- No new SSE event type, no new realtime transport, no queue/broker dependency.
- Full server (1085/58) and client (833+/68, two known-flaky unrelated timeouts under machine contention) test suites green; server + client `tsc`/`eslint`/build clean; `git diff --check` clean.
- Deferred, unchanged from plan.md: external outbound attachment transmission for Email/SMS/WhatsApp, customer merge/dedup, generic queues, realtime transport replacement, WhatsApp media/templates/reactions, read receipts/typing/presence, live-provider/production-scheduler certification.
