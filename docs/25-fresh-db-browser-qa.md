...
| **SMS (TextBee)** | CODE VERIFIED | POST webhook unconfigured → **503**. Inbound contract unchanged and still matches `sms.signature.ts`: `X-Signature` = HMAC-SHA256 over the raw request body keyed by the webhook secret. Outbound reply path now shares the post-commit seam (**Bug 1 resolved** / ADR-052): failure → **201 + `delivery.status = "FAILED"` + `SMS_DELIVERY_FAILED` history row**, reply persisted. The outbound send has a real `AbortSignal.timeout(20_000)`; a timeout or a never-connected request (DNS / socket failure) is now classified as `PROVIDER_UNREACHABLE`, consistent with Email — a TextBee that responds and rejects stays `PROVIDER_REJECTED`. Seed now produces SMS-channel tickets (**Bug 2 resolved**). Live delivery: **NOT VERIFIED**. |
...
- **Follow-up (same branch):** the Resend outbound send is now explicitly bounded to **20 seconds** (`EMAIL_DELIVERY_TIMEOUT_MS`, matching the SMS `AbortSignal.timeout(20_000)`) with a real `AbortSignal` request cancellation. A timeout is classified as **`PROVIDER_UNREACHABLE`** (never `PROVIDER_REJECTED`) → 201 + `delivery.status = "FAILED"` + one `EMAIL_DELIVERY_FAILED` history row; the already-committed CRM reply, first-response state, notifications, and realtime event are unaffected.
- **Follow-up (same branch):** SMS timeout / network-unreachable failures now map to **`PROVIDER_UNREACHABLE`**, consistent with Email — previously an `AbortSignal.timeout(20_000)` firing was collapsed into `SMS_DELIVERY_FAILED` → `PROVIDER_REJECTED`. `textBeeProvider.sendMessage` throws `AppError(504, "SMS_DELIVERY_UNREACHABLE")` for the timeout (`DOMException` "TimeoutError") and a raw connection failure (`TypeError: fetch failed`); `outboundFailureReason` maps it to `PROVIDER_UNREACHABLE`. A TextBee that responds and rejects (non-2xx / `data.success === false` / `failureCount > 0`) still maps to `PROVIDER_REJECTED`. Missing config → `INTEGRATION_NOT_CONFIGURED`, missing phone → `NO_RECIPIENT_PHONE`, success → `SENT` — unchanged. 20 s bound, transaction boundary, delivery timing, retry behaviour, WhatsApp: unchanged.
- **Regression tests:** `ticket.test.ts` (EMAIL/SMS unconfigured, runtime failure, success, no-provider-call-before-commit, single-side-effect; + SMS timeout → 201 + `PROVIDER_UNREACHABLE`), `outbound-delivery.test.ts` (reason mapping incl. `EMAIL_DELIVERY_TIMEOUT` and `SMS_DELIVERY_UNREACHABLE` → `PROVIDER_UNREACHABLE`, history-row write, SMS timeout wrapper writes exactly one row), `sms.test.ts` (+4: `AbortSignal.timeout` abort and raw connection failure → `SMS_DELIVERY_UNREACHABLE`; HTTP 5xx and body-level failure stay `SMS_DELIVERY_FAILED`), `email.test.ts` outbound block (+ timeout→`PROVIDER_UNREACHABLE`), `email.client.test.ts` (`sendTicketEmail` timeout seam — already-aborted signal, no real wait). Full server vitest **892/892** (51 files); client **767/767**.
