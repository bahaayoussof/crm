...
- **Reply tab** — the composer. A small `Reply | Internal note` mode sub-selector (`role="tablist"`, **not** a 5th top tab); a heading + helper with `Insert quick reply` at its top-right; the editor; then a `border-t pt-3` footer with a **plain icon+text `Attach file` link** (paperclip, no border/fill, real `<button>` semantics) at inline-start and the primary **`Reply`** button at inline-end. No selected-file chips ever appear in the footer.
  ...
  - **Dedicated Shared File Upload Modal:** clicking `Attach file` in the composer footer (or within the Attachments tab) opens the dedicated portalled `FileUploadModal` (`role="dialog"`, `aria-modal="true"`) outside the conversation message viewport. The conversation timeline stays fully mounted in place and is never displaced. The modal features a drag-and-drop zone, file picker trigger, accepted format and size guidance (JPEG, PNG, WebP, PDF, TXT; 4 MiB max), a formatted selected-file preview card (`SelectedFileRow`) with file-type badge, direction-isolated filename, formatted size, and remove button, alongside Cancel and Upload actions. Selecting or dropping a file stages it; clicking Upload executes the mutation, reports validation/API errors inline, and dismisses the modal on success while preserving the conversation timeline.
...
The Reply panel is only ever shown/hidden with a `hidden` class — never unmounted — so the Lexical draft, undo history, Quick Reply insertion and the AI "Insert into Reply" handle survive every tab switch and modal upload.

**Reply editor = Lexical** (`ticket-reply-editor.tsx` + `ticket-reply-toolbar.tsx` + `ticket-reply-link-popover.tsx`). Minimal support-desk formatting only: **Bold / Italic / Underline / bulleted list / numbered list / link / undo / redo** — icon-only toolbar buttons with `aria-label` + `aria-pressed`, no headings / code blocks / tables / markdown / images / slash commands. Clicking the **Link** toolbar button opens an anchored CRM popover dialog (`TicketReplyLinkPopover`, portalled to `document.body` via `useAnchoredPopover`) with URL input (`dir="ltr"`), Text input (pre-populated with selected text), "Open link in new tab" checkbox, URL validation (allowing http/https/mailto/tel and auto-prefixing https:// for naked domains while strictly rejecting dangerous schemes with inline `role="alert"` errors), existing link editing / unlink, and Escape/outside dismissal returning focus to the editor. The `contentEditable` region is `role="textbox"`, `min-h-[7rem] max-h-60 overflow-y-auto`. The internal note remains a plain `MentionTextarea` controlled by React state.
...
**Content field (ADR-057, `KB-RICH-*`).** Content is authored in a **bounded
Lexical rich-text editor** (not a `<textarea>`), built on the same
infrastructure as the ticket reply composer. V1 toolbar: paragraph, Heading 2,
Heading 3, bold, italic, underline, bulleted list, numbered list, link
(safe-protocol popover), undo/redo — nothing else (no images, media, tables,
code, quotes, colour, alignment, or raw HTML). The editor is a UX affordance
only; RBAC is unchanged (`ADMIN`/`MANAGER` reach the screen; `AGENT` cannot).
On submit the body is serialized to HTML and sanitized authoritatively on the
server. Opening a legacy plain-text article hydrates it into editable
paragraphs; saving it lazily stores it as sanitized HTML. Content validation
is on the readable (tag-stripped) length: non-empty, ≤ 50 000 characters.
...
- **Dedicated Shared File Upload Modal**: identical to internal — the composer's paperclip button and attachments panel trigger open the portalled `FileUploadModal` outside the conversation viewport; drag-and-drop or browse stages the file; clicking Upload executes the portal upload mutation; Cancel and success dismiss the modal while keeping the conversation messages timeline mounted.
...
**Ticket Details shares the internal Ticket Details visual language** (`feature/customer-ticket-details-redesign`, 2-column workspace redesign). The Customer Portal keeps its own header/navigation shell, its ownership-safe Portal APIs, and its customer-only data, composing the role-neutral presentational components as a modern, compact two-column ticket workspace:

- **Layout Structure:** On desktop (`lg+`), uses a two-column workspace grid (`minmax(0, 1fr)` main column + `300px` sticky right sidebar, `xl:grid-cols-[minmax(0,1fr)_320px]`). On mobile/tablet, single-column responsive flow with the sidebar placed naturally beneath the main workspace.
- **Header:** compact `TicketDetailHeader` with back link to My Requests, Ticket ID (`#<ref>`, LTR-isolated), subject `h1` with `break-words [overflow-wrap:anywhere]`, and `PortalStatus` badge. Customer-visible fields only. No internal actions or staff metadata.
- **Ticket Details Sidebar:** A dedicated card in the right rail containing Category (`Tag` icon + name), full Description (with `whitespace-pre-wrap break-words [overflow-wrap:anywhere]`), Created timestamp (`CalendarClock` icon + locale date/time with `<bdi dir="ltr">`), and Last Updated timestamp.
- **Conversation:** Bounded desktop height (`lg:h-[440px] lg:min-h-[360px] lg:max-h-[540px]` with `overflow-y-auto`) with a header displaying message count ("N messages"). Messages render as content-sized chat bubbles (`w-fit max-w-[85%] sm:max-w-[min(70%,560px)]`) with compact padding (`px-3.5 py-2.5`), aligned to the logical **start** for Support Team and **end** for Customer ("You") (viewer-relative alignment). Long messages progressively disclose with `Show more` / `Show less`.
- **Lower Workspace Tabs:** One unified card beneath the conversation with `role="tablist"` tabs:
  - **Reply tab:** rich Lexical editor (`TicketReplyEditor`) with compact initial height (~112px) and toolbar (bold, italic, underline, lists, link, undo, redo). Reopen notice when `RESOLVED`; calm closed notice when `CLOSED`. Footer with paperclip native file picker `Attach file` at start and primary `Reply` button at end. Editor remains mounted when switching tabs so draft text is never lost.
  - **Attachments tab:** compact attachment cards (`AttachmentCompactGrid`, `scope="portal"`) with file type icon, filename, preview button (opens `AttachmentPreviewDialog`), and download button. Shows "View all / Show less" toggle when > 3 attachments, and clean empty state when 0 attachments.
- **Customer Feedback:** Rendered inside the right rail sidebar when present or eligible. Shows star rating + optional comment + submission form when eligible, or read-only star rating + comment + submitted date when submitted.
- **Attachment Upload Flow:** Clicking `Attach file` triggers the native OS picker; upon choosing a valid file, the conversation message viewport smoothly swaps for `AttachmentWorkspace` (pre-filled with the chosen file) without displacing the composer or causing page scroll. Cancel or successful upload returns to the message viewport.
...
Detail page: back link, title, status badge, category, localized updated date (LTR-isolated), safe `{ id, name, role }` author summary, and the full article body rendered through the shared `<ArticleContent>` guard (ADR-057, `KB-RICH-*`): a server-sanitized rich body is re-sanitized on the client (DOMPurify, V1 allowlist `p/br/h2/h3/ul/ol/li/b/strong/i/em/u/a`, `http`/`https`/`mailto` links forced to safe `rel`/`target`) and injected; a legacy plain-text body renders on the unchanged `whitespace-pre-wrap`/`break-words`/`dir="auto"` path. No page-level horizontal overflow; no new rich-text dependency. `ADMIN`/`MANAGER` see `Edit` and `Delete`; `AGENT` sees neither and no mutation control. Delete uses an inline accessible confirmation region (`role="alertdialog"`) with explicit Confirm/Cancel, keyboard operable, duplicate-request-safe, with localized pending and failure states; success invalidates internal list/detail and Portal article queries and navigates to `/knowledge-base`; failure preserves the article and shows a localized error.

Editor (`/knowledge-base/new`, `/knowledge-base/:id/edit`, `ADMIN`/`MANAGER` only): React Hook Form + Zod, localized labels and validation, visible required indicators on Title and Content, a **bounded Lexical rich-text editor** for the body (ADR-057, `KB-RICH-*`) — toolbar: paragraph, H2, H3, bold, italic, underline, bulleted/numbered list, safe-protocol link popover, undo/redo; icon-only buttons with `aria-pressed`, EN/AR localized, RTL-safe (`dir="auto"` content). Content validation is on the readable tag-stripped length (non-empty, ≤ 50 000). Opening a legacy plain-text article hydrates it into paragraphs; on submit the body is serialized to HTML and the server sanitizes it authoritatively. Plus a Draft/Published `select`, initial loading state for edit, a disabled pending Save that blocks duplicate submits, API validation and failure display, cancel/back navigation. A successful create navigates to the new article; a successful update returns to article detail (which refetches).

Customer Portal: `/portal/knowledge-base` (Help Center) and `/portal/knowledge-base/:id`, with a "Help Center" nav item in the Portal shell. The list shows published article cards (title, optional category chip, server-derived excerpt, updated date) under a neutral "Help articles" heading — no "Popular" label, because the schema has no popularity/view data (documented limitation). It has search, an optional category filter, server-side pagination when the result set exceeds one page, and loading/error-retry/empty/no-results states. The detail view shows title, category, updated date, and the article body rendered through the same shared `<ArticleContent>` guard as the internal detail page (client re-sanitized rich HTML, or the unchanged plain-text path for legacy bodies); the list card `excerpt` stays server-derived plain text. The Portal never renders Draft/Published status, author identity, or Create/Edit/Delete controls; a `DRAFT` or missing article id yields the same localized not-found state.
...
`ADMIN`-only administrative workspace using the shared DataTable system. Desktop columns are Time, Actor, Action, Entity, Changes, and Context; mobile uses the canonical responsive card rendering. Search and action/entity/date filters are URL-backed and server-side. Human actors show safe name/email; null actors render as System. Technical IDs, email, IP, and User-Agent use direction-isolated presentation.

Rows expose an accessible details sheet containing only the safe response projection. Changes are compact `from → to` summaries, never raw JSON dumps. Loading, retry, no-activity, filtered-no-results, pagination, English/Arabic parity, and RTL are required. The navigation item and route are absent/guarded for `MANAGER`, `AGENT`, and `CUSTOMER`; the Customer Portal exposes no Audit Logs surface.
