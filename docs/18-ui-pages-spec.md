...
Feedback, notifications, profile editing, and realtime updates remain deferred. Owned-ticket attachments are implemented on the uncommitted `feature/attachments` branch — see "Attachments implemented behavior" below. The Portal Knowledge Base (Help Center) is implemented — see "Knowledge Base implemented behavior" below.
