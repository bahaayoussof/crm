...
export function KnowledgeBasePage({ children }: PropsWithChildren) {
...
export function ArticleStatusBadge({ status }: { status: KnowledgeArticleStatus }) {
  const { t } = useTranslation();
  const tone = status === "PUBLISHED" ? "bg-emerald-50 text-emerald-700" : "bg-muted text-muted-foreground";
  return <span className={`inline-flex w-fit rounded-md px-2 py-1 text-xs font-semibold ${tone}`}>{t(`knowledgeBase.status.${status}`)}</span>;
}
