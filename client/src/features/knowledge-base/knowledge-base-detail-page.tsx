...
export function KnowledgeBaseDetailPage() {
  ...
  const back = <Link className="rounded-sm text-sm font-medium text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/30" to="/knowledge-base">{t("knowledgeBase.backToList")}</Link>;
  ...
  return <KnowledgeBasePage>
    ...
    <PageHeader
      title={data.title}
      description={<div className="flex flex-wrap items-center gap-x-4 gap-y-2">
    ...
    {confirmingDelete && canManage && <section className="mt-5 rounded-md border border-red-200 bg-red-50 p-4" role="alertdialog" aria-labelledby="kb-delete-title" aria-describedby="kb-delete-desc">
      <h2 className="text-sm font-semibold text-red-800" id="kb-delete-title">{t("knowledgeBase.deleteTitle")}</h2>
      <p className="mt-1 text-sm text-red-700" id="kb-delete-desc">{t("knowledgeBase.deleteConfirm")}</p>
    ...
    <article className="mt-6 max-w-3xl whitespace-pre-wrap break-words text-sm leading-7 text-foreground" dir="auto">{data.content}</article>
  </KnowledgeBasePage>;
}
