...
export function ProtectedPlaceholderPage({ area }: { area: "dashboard" | "portal" }) {
  ...
  return <main className="grid min-h-screen place-items-center px-4"><section className="text-center"><h1 className="text-2xl font-semibold">{t(`auth.${area}Placeholder`)}</h1><p className="mt-2 text-muted-foreground">{t("auth.signedInAs", { name: user?.name })}</p><button className="mt-6 text-sm font-medium text-primary" onClick={() => { logout(); navigate("/login", { replace: true }); }}>{t("auth.logout")}</button></section></main>;
}
