...
export function TicketFormPage() {
  ...
  const submit = handleSubmit(async (values) => { setApiError(null); try {
    const saved = editing ? await update.mutateAsync({ subject: values.subject, description: values.description, ...(canChangePriority && { priority: values.priority }), categoryId: values.categoryId || null, ...(canAssign && { assignedAgentId: values.assignedAgentId || null }) }) : await create.mutateAsync({ ...values, categoryId: values.categoryId || null, ...(canAssign ? { assignedAgentId: values.assignedAgentId || null } : { assignedAgentId: null }) });
