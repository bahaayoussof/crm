...
describe("DashboardPage", () => {
  ...
  it("shows the role-aware primary heading", () => { renderPage(); expect(screen.getByRole("heading", { name: "My Assigned Tickets" })).toBeInTheDocument(); for (const role of ["ADMIN", "MANAGER"]) { mocks.useAuth.mockReturnValue({ user: { id: role.toLowerCase(), role } }); const view = renderPage(); expect(screen.getByRole("heading", { name: "Needs attention" })).toBeInTheDocument(); view.unmount(); } });
  ...
  it("renders independent localized empty section and distribution states", () => { mocks.useDashboard.mockReturnValue({ isLoading: false, isError: false, data: { ...data, metrics: { ...data.metrics, openTickets: 0, slaBreached: 0, slaAtRisk: 0 }, statusDistribution: [], primaryTickets: [], recentTickets: [] }, refetch: mocks.refetch }); renderPage(); expect(screen.getByText("No ticket status data yet.")).toBeInTheDocument(); expect(screen.getByText("No active tickets requiring SLA tracking.")).toBeInTheDocument(); expect(screen.getByText("No active tickets are currently assigned to you.")).toBeInTheDocument(); expect(screen.getByText("No recent visible tickets remain outside the primary queue.")).toBeInTheDocument(); });
