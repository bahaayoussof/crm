  ...
  return (
    <CustomerPage>
      <div className="space-y-6">
        ...
        <form className="max-w-2xl rounded-xl border border-border bg-surface shadow-subtle" onSubmit={submit} noValidate>
          <div className="space-y-5 p-5 sm:p-6">
            ...
            <FormField id="customer-name" label={t("customers.name")} error={errors.name?.message ? t(errors.name.message) : undefined}>
              <input id="customer-name" className="input" autoComplete="name" aria-invalid={Boolean(errors.name)} aria-describedby={errors.name ? "customer-name-error" : undefined} {...register("name")} />
            </FormField>
            <FormField id="customer-email" label={t("customers.email")} error={errors.email?.message ? t(errors.email.message) : undefined}>
          ...
          <div className="flex flex-col-reverse gap-3 border-t border-border bg-surface-subtle/40 px-5 py-4 sm:flex-row sm:justify-end sm:px-6 rounded-b-xl">
            <Link className="button-secondary text-center" to={isEditing ? `/customers/${id}` : "/customers"}>{t("common.cancel")}</Link>
            <button className="button-link" type="submit" disabled={isSubmitting}>{isSubmitting ? t("common.saving") : t("common.save")}</button>
          </div>
        </form>
      </div>
    </CustomerPage>
