...
describe("settings API", () => {
  beforeEach(() => { vi.clearAllMocks(); mocks.user.mockImplementation(({ where }: { where: { id: string } }) => Promise.resolve({ role: where.id.toUpperCase() as Role, isActive: true })); mocks.categoryFindMany.mockResolvedValue([]); mocks.categoryFindUnique.mockResolvedValue({ id: "cd0f631ca1ddba8db3bcfcb9e" }); mocks.categoryCreate.mockResolvedValue({ id: "cd0f631ca1ddba8db3bcfcb9e", name: "Billing", description: null, isActive: true }); mocks.categoryUpdate.mockResolvedValue({ id: "cd0f631ca1ddba8db3bcfcb9e", name: "Billing", description: null, isActive: false }); mocks.slaFindMany.mockResolvedValue([]); mocks.slaUpsert.mockResolvedValue({ id: "s1", priority: TicketPriority.HIGH, firstResponseMinutes: 60, resolutionMinutes: 240, isActive: true }); });
