...
const customerToken = token("customer-user-1", Role.CUSTOMER);
...
describe("attachment auth boundary", () => {
  it("rejects unauthenticated internal requests with 401", async () => {
    expect((await request(app).get("/api/tickets/ticket-1/attachments")).status).toBe(401);
    expect((await request(app).post("/api/tickets/ticket-1/attachments")).status).toBe(401);
    expect((await request(app).get("/api/attachments/att-1/download")).status).toBe(401);
    expect((await request(app).get("/api/customers/cust-1/attachments")).status).toBe(401);
  });

  it("rejects CUSTOMER from every internal attachment route with 403", async () => {
    expect((await request(app).get("/api/tickets/ticket-1/attachments").set(auth(customerToken))).status).toBe(403);
    expect((await request(app).post("/api/tickets/ticket-1/attachments").set(auth(customerToken))).status).toBe(403);
    expect((await request(app).get("/api/customers/cust-1/attachments").set(auth(customerToken))).status).toBe(403);
    expect((await request(app).post("/api/customers/cust-1/attachments").set(auth(customerToken))).status).toBe(403);
    expect((await request(app).get("/api/attachments/att-1/download").set(auth(customerToken))).status).toBe(403);
  });

  it("rejects internal roles from the Portal attachment routes with 403", async () => {
    expect((await request(app).get("/api/portal/tickets/ticket-1/attachments").set(auth(agentToken))).status).toBe(403);
    expect((await request(app).get("/api/portal/attachments/att-1/download").set(auth(adminToken))).status).toBe(403);
  });
});
...
describe("internal ticket attachment upload", () => {
  it.each([
    ...
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "OPEN", assignedAgentId: "agent-9" });
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(value)), PNG);
    expect(response.status).toBe(201);
    expect(response.body.data).toMatchObject({ fileName: "file.png", mimeType: "image/png", ticketId: "ticket-1", messageId: null, customerId: null });
  ...
  it("lets the assigned AGENT upload a ticket attachment", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "OPEN", assignedAgentId: "agent-1" });
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(agentToken)), PDF, { filename: "r.pdf", contentType: "application/pdf" });
    expect(response.status).toBe(201);
  ...
  it("rejects an AGENT uploading to an unassigned ticket with 403", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "OPEN", assignedAgentId: null });
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(agentToken)), PNG);
  ...
  it("rejects an AGENT uploading to another AGENT's ticket with 403", async () => {
    ...
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(otherAgentToken)), PNG);
  ...
  it("applies ticket visibility to listing (hidden ticket -> 404)", async () => {
    mocks.ticketFindFirst.mockResolvedValue(null);
    const response = await request(app).get("/api/tickets/ticket-x/attachments").set(auth(agentToken));
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe("TICKET_NOT_FOUND");
  });
  ...
  it("lists ticket-level and message-level attachments once each", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "OPEN", assignedAgentId: null });
    mocks.attachmentFindMany.mockResolvedValue([
      createdRow({ id: "a1", messageId: null }),
      createdRow({ id: "a2", messageId: "msg-1" }),
    ]);
    const response = await request(app).get("/api/tickets/ticket-1/attachments").set(auth(adminToken));
    expect(response.status).toBe(200);
    expect(response.body.data.map((a: { id: string }) => a.id)).toEqual(["a1", "a2"]);
    const where = mocks.attachmentFindMany.mock.calls[0]?.[0].where;
    expect(where).toEqual({ OR: [{ ticketId: "ticket-1", messageId: null }, { message: { ticketId: "ticket-1" } }] });
  });
});
...
describe("customer-profile attachments", () => {
  it.each([
    ...
    mocks.customerFindUnique.mockResolvedValue({ id: "cust-1" });
    const response = await attach(request(app).post("/api/customers/cust-1/attachments").set(auth(value)), TXT, { filename: "note.txt", contentType: "text/plain" });
    expect(response.status).toBe(201);
    expect(response.body.data).toMatchObject({ mimeType: "text/plain", customerId: "cust-1", ticketId: null, messageId: null });
  });

  it("rejects an AGENT uploading a customer attachment with 403", async () => {
    const response = await attach(request(app).post("/api/customers/cust-1/attachments").set(auth(agentToken)), TXT, { filename: "note.txt", contentType: "text/plain" });
    expect(response.status).toBe(403);
    expect(mocks.attachmentCreate).not.toHaveBeenCalled();
  });
  ...
  it("lets an AGENT list and download a customer attachment", async () => {
    mocks.customerFindUnique.mockResolvedValue({ id: "cust-1" });
    mocks.attachmentFindMany.mockResolvedValue([createdRow({ id: "a1", ticketId: null, customerId: "cust-1" })]);
    const list = await request(app).get("/api/customers/cust-1/attachments").set(auth(agentToken));
    expect(list.status).toBe(200);
    expect(list.body.data).toHaveLength(1);

    storage.seed("attachments/key-c", TXT, "text/plain");
    mocks.attachmentFindUnique.mockResolvedValue({ fileName: "note.txt", mimeType: "text/plain", storageKey: "attachments/key-c", ticketId: null, messageId: null, customerId: "cust-1" });
    const download = await request(app).get("/api/attachments/a1/download").set(auth(agentToken));
...
describe("message attachment context", () => {
  it("requires the message to belong to the supplied ticket", async () => {
    ...
    const response = await attach(request(app).post("/api/tickets/ticket-1/messages/msg-x/attachments").set(auth(adminToken)), PNG);
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe("MESSAGE_NOT_FOUND");
    const where = mocks.ticketMessageFindFirst.mock.calls[0]?.[0].where;
    expect(where).toEqual({ id: "msg-x", ticketId: "ticket-1" });
  });

  it("rejects attaching to another user's message with 403 for ADMIN too", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "OPEN", assignedAgentId: null });
    mocks.ticketMessageFindFirst.mockResolvedValue({ id: "msg-1", authorUserId: "someone-else" });
    const response = await attach(request(app).post("/api/tickets/ticket-1/messages/msg-1/attachments").set(auth(adminToken)), PNG);
    expect(response.status).toBe(403);
    expect(response.body.error.code).toBe("FORBIDDEN");
    expect(mocks.attachmentCreate).not.toHaveBeenCalled();
  });
  ...
  it("lets the author attach to their own message", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "OPEN", assignedAgentId: "agent-1" });
    mocks.ticketMessageFindFirst.mockResolvedValue({ id: "msg-1", authorUserId: "agent-1" });
    const response = await attach(request(app).post("/api/tickets/ticket-1/messages/msg-1/attachments").set(auth(agentToken)), PNG);
    expect(response.status).toBe(201);
    expect(response.body.data).toMatchObject({ ticketId: "ticket-1", messageId: "msg-1" });
  });
});
...
describe("upload validation", () => {
  ...
  it("accepts a request whose only part is the file", async () => {
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), PNG);
    expect(response.status).toBe(201);
  ...
  it("rejects an empty file", async () => {
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), Buffer.alloc(0), { filename: "empty.txt", contentType: "text/plain" });
    expect(response.status).toBe(422);
  ...
  it("rejects a file larger than 4 MiB with FILE_TOO_LARGE", async () => {
    ...
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), big, { filename: "big.txt", contentType: "text/plain" });
  ...
  it("rejects an unsupported MIME type by content", async () => {
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), EXE, { filename: "tool.png", contentType: "image/png" });
  ...
  it("rejects MIME spoofing (HTML body claimed as text/plain)", async () => {
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), HTML, { filename: "notes.txt", contentType: "text/plain" });
    expect(response.status).toBe(415);
    expect(response.body.error.code).toBe("UNSUPPORTED_FILE_TYPE");
  });

  it("stores the detected type, not the client-claimed type", async () => {
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), PNG, { filename: "x.pdf", contentType: "application/pdf" });
    expect(response.status).toBe(201);
    expect(response.body.data.mimeType).toBe("image/png");
  });

  it("normalizes an unsafe filename (strips path traversal)", async () => {
    const response = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), PDF, { filename: "../../../etc/passwd.pdf", contentType: "application/pdf" });
    expect(response.status).toBe(201);
    expect(response.body.data.fileName).toBe("passwd.pdf");
  });

  it("generates unpredictable, server-derived storage keys", async () => {
    await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), PNG);
    await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), PNG);
...
describe("failure handling and safe responses", () => {
  ...
  it("never exposes storageKey or provider URLs in list or upload responses", async () => {
    mocks.attachmentFindMany.mockResolvedValue([createdRow({ id: "a1" })]);
    const list = await request(app).get("/api/tickets/ticket-1/attachments").set(auth(adminToken));
    const upload = await attach(request(app).post("/api/tickets/ticket-1/attachments").set(auth(adminToken)), PNG);
...
describe("download security", () => {
  it("returns safe headers for an authorized download", async () => {
    ...
    mocks.attachmentFindUnique.mockResolvedValue({ fileName: "diagram.png", mimeType: "image/png", storageKey: "attachments/key-1", ticketId: "ticket-1", messageId: null, customerId: null });
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1" });
    const response = await request(app).get("/api/attachments/att-1/download").set(auth(adminToken));
    expect(response.status).toBe(200);
  ...
  it("authorizes before any storage retrieval (hidden ticket -> 404, get untouched)", async () => {
    ...
    const response = await request(app).get("/api/attachments/att-1/download").set(auth(agentToken));
    expect(response.status).toBe(404);
  ...
  it("returns a structured 404 for a missing attachment record", async () => {
    ...
    const response = await request(app).get("/api/attachments/missing/download").set(auth(adminToken));
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe("ATTACHMENT_NOT_FOUND");
  });
  ...
  it("returns ATTACHMENT_NOT_FOUND (no provider detail) when the stored object is missing", async () => {
    const getSpy = vi.spyOn(storage, "get");
    mocks.attachmentFindUnique.mockResolvedValue({ fileName: "x.png", mimeType: "image/png", storageKey: "attachments/gone", ticketId: "ticket-1", messageId: null, customerId: null });
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1" });
    const response = await request(app).get("/api/attachments/att-1/download").set(auth(adminToken));
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe("ATTACHMENT_NOT_FOUND");
  ...
  it("rejects a stored object larger than 4 MiB and never reads its bytes", async () => {
    ...
    mocks.attachmentFindUnique.mockResolvedValue({ fileName: "huge.txt", mimeType: "text/plain", storageKey: "attachments/huge", ticketId: "ticket-1", messageId: null, customerId: null });
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1" });
    const response = await request(app).get("/api/attachments/att-1/download").set(auth(adminToken));
    expect(response.status).toBe(413);
    expect(response.body.error.code).toBe("FILE_TOO_LARGE");
...
describe("Customer Portal attachments", () => {
  ...
  it("lets a CUSTOMER upload to an owned non-closed ticket without creating a message or reopening", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1", status: "RESOLVED" });
    const response = await attach(request(app).post("/api/portal/tickets/ticket-1/attachments").set(auth(customerToken)), PDF, { filename: "receipt.pdf", contentType: "application/pdf" });
    expect(response.status).toBe(201);
  ...
  it("rejects a CUSTOMER uploading to another customer's ticket with 404 TICKET_NOT_FOUND", async () => {
    ...
    const response = await attach(request(app).post("/api/portal/tickets/ticket-1/attachments").set(auth(customerToken)), PDF, { filename: "x.pdf", contentType: "application/pdf" });
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe("TICKET_NOT_FOUND");
    const where = mocks.ticketFindFirst.mock.calls[0]?.[0].where;
    expect(where).toEqual({ id: "ticket-1", customerId: "portal-cust-1" });
  });
  ...
  it("lists owned-ticket attachments with a minimal projection", async () => {
    mocks.ticketFindFirst.mockResolvedValue({ id: "ticket-1" });
    mocks.attachmentFindMany.mockResolvedValue([
      { id: "a1", fileName: "f.pdf", mimeType: "application/pdf", createdAt: new Date("2026-08-26T12:00:00.000Z"), messageId: null },
    ]);
    const response = await request(app).get("/api/portal/tickets/ticket-1/attachments").set(auth(customerToken));
    expect(response.status).toBe(200);
    expect(response.body.data[0]).toEqual({ id: "a1", fileName: "f.pdf", mimeType: "application/pdf", createdAt: "2026-08-26T12:00:00.000Z", messageId: null });
  ...
  it("rejects a CUSTOMER downloading another customer's attachment with 404", async () => {
    ...
    const response = await request(app).get("/api/portal/attachments/att-1/download").set(auth(customerToken));
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe("ATTACHMENT_NOT_FOUND");
  });
