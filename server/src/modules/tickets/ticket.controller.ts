...
export const create: RequestHandler<unknown, unknown, CreateTicketInput> = async (request, response) => response.status(201).json({ data: await createTicket(request.body, actor(request), getAuditRequestContext(request)) });
export const update: RequestHandler<unknown, unknown, UpdateTicketInput> = async (request, response) => response.status(200).json({ data: await updateTicket((response.locals.validatedParams as TicketParams).id, request.body, actor(request), getAuditRequestContext(request)) });
